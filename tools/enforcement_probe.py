...
protects the enforcement chain ITSELF: a fix to a chain file is
verified against the OBSERVER that watches the chain
(tools/wiring_check.py), not only against the symptom the fix
targeted. Motive (synk from the sibling Dog deployment, D-0082,
2026-07-29, docs/tasks/2026-07-29_dog-incoming-sync.md item 1): on
2026-07-25 Dog's own accepted fix to a chain file silently broke their
wiring_check parser for a whole day -- the fix's own DoD checked the
symptom it targeted, never the wiring auditor sitting downstream of
it, so the breakage went unnoticed until the next unrelated audit.

DELIVERY NOTE (enforcement-file-review rule, CLAUDE.md rule 2 /
routing rules): a builder dispatch never places a self-activating hook
file on its own live path. Delivered by the builder as the inert
neighbor file tools/enforcement_probe_next.py; the Lead moved it onto
this live name and wired the call into .githooks/pre-commit at
acceptance of t-338 (D-0069, 2026-07-29).
...
DESIGN (spec part E):
  1. Staged set: `git -c core.quotePath=false diff --cached
     ...
     `_staged_files` for the `--no-renames`/`quotePath` rationale,
     F2/F9(д), critic t-339). An empty staged set is exit 0
     immediately -- no git-index work happens on an ordinary commit
     that isn't touching the chain at all.
  2. The enforcement CHAIN is built DYNAMICALLY, not from a hardcoded
     list (a hardcoded list goes stale) -- see `_chain_paths`: every
     path under .githooks/ (RECURSIVELY -- F9(г), critic t-339: a
     hook script nested in a subdirectory under .githooks/ is walked
     too, not just its top level), `.claude/settings.json` itself,
     `tools/wiring_check.py` by name, and every `tools/<name>.py`
     referenced by a `python tools/....py` hook command inside
     .claude/settings.json OR inside any script under .githooks/ (the
     same two sources tools/wiring_check.py itself reads for its own
     harness/git-hooks checks -- D-0093 pattern; see "PARSER REUSE"
     below for why this file keeps its own separate reader rather than
     importing that one).
  3. Staged ∩ chain empty -> exit 0, the wiring check is NOT invoked
     at all (the cost on an ordinary commit is exactly the staged-list
     read, nothing more).
  4. Staged ∩ chain non-empty -> the wiring check is run (see
     `_default_run_wiring_check` -- `python tools/wiring_check.py` as
     a subprocess, matching this deploy's own convention of running
     tools/*.py scripts via `python tools/<name>.py`, same shape as
     every hook command already registered in .claude/settings.json).
     A non-zero exit code (or an import/parse/execution failure of any
     kind -- see `decide()`) is reported and the commit is rejected
     (exit 1); this IS genuinely fail-closed by design -- an observer
     that cannot even run is exactly the failure class this probe
     exists to catch, so silence there would defeat its own purpose.
     See "GIT-FAILURE HANDLING IS NOT THIS" below for the ONE step in
     this pipeline that is deliberately the opposite.
  5. .githooks/pre-commit is NOT touched by this delivery -- the
     coordinator wires the call in at acceptance (non-goal, per the
     dispatch's manifest).

WAY OUT OF THE DEADLOCK (F9(б), critic t-339): this probe runs
`python tools/wiring_check.py` against the WORKING TREE (a plain
subprocess invocation, not `git show :tools/wiring_check.py` or
anything reading the INDEX) -- so fixing tools/wiring_check.py (or
...
very next attempt. There is no separate "unwedge" step or override
flag needed: the observer this probe consults is the same one a
developer would fix by hand, read from the same place.

LIMITATION -- STAGED CONTENT ITSELF IS NOT CHECKED (F9(в), critic
t-339): the wiring check always runs against the CURRENT WORKING TREE
files, not the staged (about-to-be-committed) versions. A pathological
sequence -- stage a BROKEN tools/wiring_check.py, then fix the working
tree copy back to a working state WITHOUT re-staging the fix -- would
pass this probe (the working tree is healthy) while the actual commit
still carries the broken staged version. This is a known, accepted gap
(not chased preventively, same "warn/fail-closed is not an absolute
security boundary" principle the sibling hygiene_gate.py documents for
its own residuals): fixing it would require diffing the wiring check
against the STAGED blob (`git show :tools/wiring_check.py` executed in
a temp location), a meaningfully bigger mechanism than this dispatch's
scope.

RESOLVED (was a KNOWN LIMITATION at first delivery, CLAUDE.md rule 3 --
checked empirically, reported rather than silently patched around, and
resolved by the coordinator's respec): at first delivery,
tools/wiring_check.py did not exist anywhere on this deploy's tools/
path (confirmed via `git ls-files`, positive/negative control). Part D
of the originating dispatch has since landed as tools/wiring_check.py
(a thin CLI wrapping tools/session_context.py's own WIRING-INTEGRITY
channels, plus a new skills-casing channel -- see that file's own
docstring). `_default_run_wiring_check` below invokes
`python tools/wiring_check.py` by subprocess (design point 4, "or
subprocess python tools/wiring_check.py" per spec) exactly as
designed before the file existed -- NOTHING in this file changed as a
result of tools/wiring_check.py landing; this was the whole point of
taking the callable as a parameter to `decide()` in the first place.
tools/test_enforcement_probe.py now also exercises the REAL,
non-mocked `python tools/wiring_check.py` invocation against the
actual live repo (empirically confirmed WIRING: OK at the time of this
delivery), on top of the injected-callable matrix that already covered
every state (OK / non-OK / crash / chain untouched) independent of
whether the real file existed.
...
fallback. tools/wiring_check.py now exists (see "RESOLVED" above), but
it does not expose a function shaped for THIS use (build the set of
repo-relative paths that constitute the enforcement chain, for a
git-diff intersection check) -- its own hook-command parsing
(session_context._parse_hook_commands, wrapped by
session_context.harness_channel) answers a DIFFERENT question ("does
each referenced tools/*.py file exist and import cleanly?"), not
"which paths are in the chain". `_chain_paths` reads the exact same
two sources (.claude/settings.json, .githooks/*) with its own
regex-based extraction, kept deliberately separate rather than forcing
an ill-fitting reuse of a differently-shaped internal helper.

GIT-FAILURE HANDLING IS NOT THIS (F9(a), critic t-339 -- corrects an
earlier, INACCURATE claim in this docstring that `main()` was
"fail-closed at every stage"): `_staged_files()` catches its OWN git
...
non-empty, a wiring-check run that errors, crashes, or reports
...
def _chain_paths():
    ...
    F9(г)), the exact file .claude/settings.json, tools/wiring_check.py
    by name, plus every tools/*.py referenced by a hook command inside
...
def _staged_files():
    ...
    detection ON (git's default), `git mv tools/wiring_check.py
    tools/wiring_check_v2.py` can report as a SINGLE line naming only
    ...
    REMOVES the observer from tools/wiring_check.py would then not
...
def _default_run_wiring_check():
    ...
    point 4, "или subprocess python tools/wiring_check.py"). Returns
    (ok, detail): ok is True iff the process exits 0 (standard Unix
    convention, consistent with every other exit-code contract in this
    repo); detail is combined stdout+stderr text, for the rejection
    message. Any exception (script missing entirely, interpreter not
    found, timeout) is NOT caught here: it propagates to `decide()`'s
    own try/except,
    which turns it into the fail-closed exit-1 path (design point 4's
    "ошибка импорта/запуска... тоже exit 1")."""
...
def main():
    ...
    critic t-339): `_staged_files()`/`_chain_paths()` already fold
    their own known failure modes (git unavailable, unreadable chain
    sources) into an empty/partial result rather than raising -- this
    outer try/except is a narrow backstop for a genuinely UNFORESEEN
    exception escaping `decide()` itself, not this probe's primary
    fail-closed guarantee (that lives inside `decide()`, design point
    4). Never raises out of `__main__`."""
    try:
