...
def test_count_protocol_checks_real_protocol_is_36():
    """Пин числа чеков живого протокола.

    Это НАМЕРЕННАЯ растяжка, а не хрупкость: пин ловит регресс парсера
    (сломанный regex вернул бы 12, а не 36) и заодно обязывает того, кто
    ...
    heartbeat-fastdeath) уронил этот тест ровно поэтому. 2026-08-19:
    чек 34 (Phase 5 W3) поднял счёт с 34 до 35, тем же днём чек 35
    (кросс-пункт среза sibling-map AO3) -- до 36; пин обновляется тем
    же классом правки, тест переименовывается вслед за числом.
    """
    assert sm.count_protocol_checks() == 36
