...
"ПРОВЕРКИ" п.2 выше: "'owns' сам по себе -- одновременно write-признак
И один из двух манифест-маркеров, спека явно: 'если owns уже есть,
значит манифест писали'") ПОМЕЧЕНО ОШИБОЧНЫМ ОБОБЩЕНИЕМ, дата
...
в owns-декларации (см. owns_declaration_has_path_token() ниже).
...
БЫ ОДНИМ настоящим path-подобным токеном -- см. функцию ниже за
...
правка) -- см. is_path_like_token() ниже за полный контракт и
...
# F-59 подкласс 2 (2026-08-10): \bowns\b УДАЛЁН -- см. докстринг модуля,
# "F-59 ПОДКЛАСС 2", за полное обоснование пересмотра. Owns остаётся
# манифест-маркером (OWNS_WORD_RE ниже), но больше не сам по себе
# write-признак.
...
# --- t-384 (2026-08-10, критик): общий дискриминатор "путь-подобный
# токен" -- ЕДИНАЯ точка правды для (а) F6 -- owns_gate.py импортирует
# is_path_like_token() вместо своего прежнего is_path_token() (голое
# "*" без слэша -- markdown "**жирный**" -- больше НЕ путь); (б) F-59
# T1-исправления -- owns_declaration_has_path_token() ниже, дешёвый
# write-предикат проверки 2. ВЫБОР РЕАЛИЗАЦИИ (спека прямо просит
# назвать и обосновать): мини-функция ЗДЕСЬ, owns_gate.py её ИМПОРТИРУЕТ
# через УЖЕ существующий импорт (owns_gate -> dispatch_gate,
# направление не меняется, обратного импорта dispatch_gate -> owns_gate
# НЕТ -- цикл невозможен структурно). НЕ выбрана альтернатива "локальная
# копия + тест равенства двух копий" (по образцу гейт/аудит гигиены):
# та пара имеет РАЗНЫЕ роли (хук проверяет на лету, аудит перепроверяет
# журнал постфактум) и поэтому терпит дрейф между копиями как
# отдельный, документированный риск; здесь же ОБА потребителя -- ТОТ ЖЕ
# самый вопрос "это путь?" в ТОТ ЖЕ момент (PreToolUse), дрейф между
# копиями был бы чистым дефектом без компенсирующей причины (класс
# t-332/t-336: две независимые копии OWNS_WORD_RE УЖЕ разошлись бы,
# если импорт не был бы введён тогда) -- импорт единственного объекта
# дешевле и безопаснее, чем поддерживать тест равенства НАВСЕГДА.
...
def is_path_like_token(tok) -> bool:
    ...
    (импорт, заменяет его прежний локальный is_path_token()).
...
# owns, non-goals, handoff) -- ТА ЖЕ семантика стоп-условия, что
# owns_gate._SECTION_HEADER_STOP_RE (независимая, НЕ импортированная
# копия -- owns_gate НЕ может импортировать ИЗ dispatch_gate то, чего
# тут не было бы без цикла; здесь предикат идёт В ДРУГУЮ сторону,
# owns_gate копию не разделяет намеренно -- дешёвый предикат допускает
# лёгкое расхождение деталей многострочного разбора, см. докстринг
# модуля "T-384 ИСПРАВЛЕНИЕ" -- согласованность проверена ТЕСТОМ
# РАВЕНСТВА на матрице форм, не структурным разделением кода).
_OWNS_SECTION_STOP_RE = re.compile(
    r"^\s*(?:\*\*)?(given|дано|owns|non-goals|handoff)\b", re.IGNORECASE
)
# После маркера, если сама строка без пути: "декларация, не проза" --
# ТОЛЬКО опциональный parenthetical-уточнение + мусорные символы
# (двоеточие/тире/звёзды/кавычки/ёлочки/пробелы) ДО КОНЦА строки.
# Единственный проход (не итеративный, в отличие от owns_gate.
# _strip_owns_marker_junk) -- достаточно для канонических форм с ОДНОЙ
# скобочной группой; "дешёвый предикат", не полный парсер.
...
def owns_declaration_has_path_token(prompt: str) -> bool:
    """Дешёвый write-предикат проверки 2 (t-384): True, если промпт
    несёт ХОТЯ БЫ ОДНУ owns-декларацию (OWNS_WORD_RE, граница слова) с
    ХОТЯ БЫ ОДНИМ настоящим path-подобным токеном (is_path_like_token)
    -- НА ТОЙ ЖЕ строке, ЛИБО (строка маркера сама -- декларация, не
    проза посреди предложения -- см. owns_gate.py "ПЕРЕСДАЧА" за тот же
    класс) на СЛЕДУЮЩЕЙ строке сразу под маркером (одна строка
    просмотра вперёд -- owns_gate.extract_owns_paths структурно требует
    путь именно НА ПЕРВОЙ строке продолжения, иначе блок кончается
    пустым -- см. его докстринг "МНОГОСТРОЧНЫЙ OWNS-БЛОК"; поэтому
    более глубокий просмотр здесь не даёт новых True-случаев, только
    риск разойтись с owns_gate). Эталон истинности -- owns_gate.
    extract_owns_paths(prompt) непустой; согласованность на матрице
    форм (девять живых образцов, канонические манифесты, голое слово в
    прозе) проверена тестом test_owns_declaration_has_path_token_
    agrees_with_owns_gate_extract_on_matrix (test_dispatch_gate.py)."""
...
def decide(payload: dict) -> tuple[int, str]:
    ...
    if subagent_type == "builder":
        ...
        # t-384 (2026-08-10, критик): write-признак -- ЛЮБОЙ из четырёх
        # глаголов ИЛИ owns-декларация с ХОТЯ БЫ ОДНИМ настоящим
        # path-подобным токеном (см. докстринг модуля, "T-384
        # ИСПРАВЛЕНИЕ" -- голое owns БЕЗ токена по-прежнему НЕ
        # write-признак, канонический owns-манифест С путём -- снова да).
