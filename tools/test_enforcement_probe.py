"""Tests for tools/enforcement_probe.py -- fail-closed pre-commit
gate protecting the enforcement chain against a fix that silently
breaks its own observer (tools/wiring_check.py). See the module's own
docstring for the motive (Dog synk, D-0082, 2026-07-29) and the
"RESOLVED" section: tools/wiring_check.py did not exist at first
delivery (Part D of the originating dispatch was returned as a
blocking question) and now does, per the coordinator's respec -- most
tests in this file still exercise the OK/non-OK/crash matrix via an
INJECTED wiring-check callable (`decide()`'s design) or via a
throwaway fake repo with its own stub tools/wiring_check.py (coverage
that does not depend on any particular real-file state), PLUS one
real, non-mocked end-to-end test (`test_real_wiring_check_ok_real_run`)
that invokes the ACTUAL `python tools/wiring_check.py` against the
live repo (respec point 5).

DoD matrix covered (к5 of the dispatch):
  - staged empty -> exit 0.
  ...
  - chain touched + wiring OK -> exit 0 (injected callable AND a real,
    ...
    tools/wiring_check.py -- respec point 5).
...
Run from the repo root: python -m pytest tools/test_enforcement_probe.py -q
"""
import subprocess
import sys
import textwrap
...
# ---------------------------------------------------------------------
# respec point 5: real, NON-MOCKED subprocess invocation of the now-
# existing tools/wiring_check.py against the actual live repo -- proves
...
# above. `decide()` is called directly with a fabricated (staged,
# chain) pair that FORCES an intersection -- no git staging happens,
# nothing in the real repo is touched -- while `run_wiring_check` is
...
# current state (empirically WIRING: OK at the time of this delivery,
# see the delivery report's witness).
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# CLI / subprocess-level end-to-end matrix (к5), against a fully
# isolated fake repo -- the real script logic, real git, real
# subprocess invocation of tools/wiring_check.py.
# ---------------------------------------------------------------------
...
def test_cli_chain_file_renamed_via_git_mv_wiring_still_invoked(tmp_path):
    # F2 (critic t-339): `git mv` of a chain member (tools/dummy_hook.py,
    # referenced by .claude/settings.json's own hook command) must
    # still register as "chain touched" -- with git's DEFAULT rename
    # detection, `--name-only` can collapse a same-content rename into
    # a SINGLE line naming only the NEW path (not itself a chain
    # member), silently hiding that the OLD (chain) path was removed.
