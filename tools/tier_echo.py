...
КАНАЛ ВЫВОДА -- СМЕНЁН прогоном №16 (предрегистрированный ключ k6,
эмпирика на живом харнессе, не домысел): stderr при exit 0 ГЛОТАЕТСЯ
харнессом -- во всех клетках прогона с живыми SubagentStop ноль строк
"TIER ECHO" дошло до транскриптов координатора, хотя сам хук
отрабатывал и писал строку в stderr корректно (см. предыдущую версию
этого докстринга/тестов -- они были написаны ДО этой эмпирики и
проверяли ровно stderr). Видимые недирективные сообщения того же
класса (WARN tools/hygiene_gate.py) ходят другим каналом -- JSON
`hookSpecificOutput`/`additionalContext` через stdout -- и ДОХОДЯТ.
tier_echo.py теперь пишет туда же, точная форма полей скопирована с
hygiene_gate.py (его эталон для этого канала, не изобретена заново):

  {"hookSpecificOutput": {"hookEventName": "SubagentStop",
                           "additionalContext": "<строка TIER ECHO...>"}}

(hookEventName -- "SubagentStop", а не "PreventToolUse"/"PreToolUse"
hygiene_gate.py, т.к. это событие ДРУГОГО хука; сама форма ключей --
та же). Строка в additionalContext -- буквально та же build_line(...),
что раньше уходила только в stderr. Прежний sys.stderr.write(...)
СОХРАНЁН вдобавок (не заменён): спека прямо разрешала оба варианта
("вместо (или вдобавок к)"), stderr-запись не вредит (тихий канал,
никто не читает, но и не портит), а сохранение снижает риск регрессии
для любого стороннего потребителя, что уже мог парсить stderr этого
хука. Обе записи -- ТОЛЬКО когда counts непустой (п.4 main() ниже);
на всех тихих ветках (битый payload, нет transcript_path, файл не
открылся, пустой/весь-synthetic транскрипт) -- ни один канал НЕ
трогается, stdout и stderr оба остаются пустыми. Живой смок доставки
координатору через реальный харнесс (не subprocess-эмуляцию) НЕ
делался (тот же метод/то же ограничение, что у dod_gate.py/
hygiene_gate.py -- потребовал бы Task/Agent-диспатч, вне роли builder,
D-0037); финальная сверка на реальном харнессе -- за Lead (см. отчёт).
...
Логика main():
 ...
 6. Печатает строку в stderr (сохранено, см. "КАНАЛ ВЫВОДА" выше) И
    печатает JSON {"hookSpecificOutput": {"hookEventName":
    "SubagentStop", "additionalContext": "<та же строка>"}} одной
    строкой в stdout -- канал, реально доходящий до координатора
    (эмпирика №16 k6). Exit 0 всегда. Любое непредвиденное исключение
