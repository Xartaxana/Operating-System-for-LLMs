...
gate: tools/search_control_gate.py (half A) records every
observed search AND examination (Read) into a per-session ledger; this
hook reads content about to be written to an in-scope path, looks for
a NEGATIVE CLAIM ("X does not exist", "не найден", ...), extracts
...
live on half A, see tools/search_control_gate.py's own
docstring):
...
def _scan_negative_claims(text):
    ...
    BOUNDARY-SAFE occurrence of a marker in *text* (F4(1), critic
    t-339 -- see NEGATIVE_MARKERS' own docstring comment and
    `_boundary_pattern_for_marker`), window being the SENTENCE
    containing the hit (t-021 D4b + t-022 fix; see `_sentence_window`).
    A lone `\\n` inside the window (a mere wrap, not a boundary) is
    normalized to a space before the window is handed to token
    extraction -- see this section's "t-022 FIX" note above for what
    this does and does not fix. Multiple occurrences of the same
...
def _find_claim_token_groups(text):
    ...
    group extracted from every negative-claim sentence in *text* (t-021
    D1 + D4b). The first marker found for a given group wins, for the
