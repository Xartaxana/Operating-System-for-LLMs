...
def test_boot_budget_normal_under_warn_threshold(tmp_path):
    ...
    assert lines == [
        "BOOT BUDGET: 300 bytes / 100000 (2 files) | CLAUDE.md: 200/37500"
        " RATCHET SAG 37200 B: WARN 37500 > ceil(200*1.015) = 300 -> re-derive (lower is free)"
    ]
...
def test_boot_budget_warn_includes_top3(tmp_path):
    ...
    assert lines[0] == (
        f"BOOT BUDGET: {total} bytes / 100000 (4 files) WARN | CLAUDE.md: 100/37500"
        " RATCHET SAG 37300 B: WARN 37500 > ceil(100*1.015) = 200 -> re-derive (lower is free)"
    )
...
def test_boot_budget_breach_includes_hint_and_top3(tmp_path):
    ...
    assert lines[0] == (
        ...
        "(D-0068; report first, operator word starts it) | CLAUDE.md: 100/37500"
        " RATCHET SAG 37300 B: WARN 37500 > ceil(100*1.015) = 200 -> re-derive (lower is free)"
    )
...
def test_boot_budget_missing_file_counts_zero_and_is_flagged(tmp_path):
    ...
    assert lines[0] == (
        ...
        " | CLAUDE.md: 50/37500"
        " RATCHET SAG 37400 B: WARN 37500 > ceil(50*1.015) = 100 -> re-derive (lower is free)"
    )
