"""SessionStart hook: prints measured reality into a new session's
context (t-027, F-30 layer 2: "reality in the background" -- a fresh
session should not have to ask "what time is it / is the journal open /
how stale is calibration / how much quota headroom is left" before it
can trust its own boot picture).

Hard constraints (all from the spec, all load-bearing):
...
- Fast (<2s) and NO network at all (the NOW line's whole point is
  ...
  D-0067's ts-from-narrative defect, journal 2026-07-10T08:10:33
  defect_found, was exactly a session writing time from the story
  instead of asking the clock).
- ASCII-safe output: this environment's console is cp1251, and this
  project has hit real breakage from non-ASCII stdout before. Every
  line built here is plain ASCII (weekday names are looked up from a
  hardcoded English table, not datetime's locale-dependent %A, which
  would print Cyrillic under a Russian locale and violate this
  constraint silently).
- <=25 lines total.
...
def build_context_lines(root: Path = None, now: datetime.datetime = None) -> list:
    ...
    lines = [now_line(now), last_event_line(events)]
...
def main(root: Path = None) -> int:
    ...
    anywhere in build_context_lines(), discards everything gathered so
    far and prints only the warning line."""
    try:
        for line in build_context_lines(root):
            print(line)
