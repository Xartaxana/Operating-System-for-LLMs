...
тул), если file_path -- харнесс-scratchpad (подстрока "scratchpad" в
пути, регистронезависимо -- буквально паттерн из системного промпта
builder'а: "Scratchpad Directory", временный каталог сессии ВНЕ
репозитория) ИЛИ путь целиком лежит вне cwd (корня репо). Правки туда
(временные скрипты харнесса, черновые файлы разбора) исключены из
main-edit-скоупа ЦЕЛИКОМ -- main_gate.py/dod_gate.py их не видят ни
для doc-only исключения, ни для green-инварианта, будто их не было.
Неизвестный file_path/cwd -- КОНСЕРВАТИВНО НЕ scratchpad (fail-safe,
симметрично doc-only-трактовке неизвестного расширения в
main_gate.py/dod_gate.py): отсутствие информации не даёт права на
исключение. См. _is_scratchpad_path().
"""
...
# (main_gate.py/dod_gate.py). Литеральный паттерн "scratchpad" в пути --
# буквально спека т-278 ("паттерн пути с 'scratchpad'"). Case-insensitive:
# путь на Windows может прийти в любом регистре диска/сегментов.
_SCRATCHPAD_PATTERN = "scratchpad"


def _is_scratchpad_path(file_path, cwd) -> bool:
    """True, если file_path -- харнесс-scratchpad ('scratchpad' в пути,
    case-insensitive) ИЛИ путь целиком лежит ВНЕ корня репо (cwd) --
    буквально вторая половина спеки т-278 п.3 ("вне корня репо").
    Консервативная трактовка (симметрично _is_doc_only_file): при
    НЕИЗВЕСТНОМ file_path/cwd -- False (НЕ исключается, инвариант в
    силе) -- отсутствие информации не даёт права на исключение из
    main-edit-скоупа, тот же fail-safe принцип, что doc-only-исключение
    для неизвестного расширения."""
