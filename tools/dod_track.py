...
test_a8_path_occupied_by_directory[dod_track] красная -- "путь занят
КАТАЛОГОМ" (не файлом) на СУЩЕСТВУЮЩИЙ путь: path.exists() -> True
(каталог существует), но path.read_text() поднимает PermissionError
(Windows-эмпирика: попытка открыть каталог как файл на чтение даёт
[Errno 13], НЕ IsADirectoryError POSIX-формы, но тот же класс) --
ДО этой правки этот OSError ловился ТЕМ ЖЕ широким `except Exception`,
что и JSONDecodeError, и уходил в _quarantine_bad_track(path), которая
делала tempfile.mkstemp() (успешно, каталог-родитель доступен) + затем
os.replace(str(path), tmp_name) -- ЭМПИРИЧЕСКИ ПРОВЕРЕНО (builder,
2026-08-19): os.replace() каталога поверх существующего файла на этой
Windows-машине УСПЕВАЕТ и переименовывает каталог ПРОЧЬ (боевой путь
после этого -- уже не каталог) -- нарушение регресс-пина A8 узла A
F-61 ("каталог не должен исчезнуть/замениться файлом при отказе").
ФИКС (решение Lead, t-531): ЧТЕНИЕ разделено на ДВА шага -- (1)
`path.read_bytes()` СНАРУЖИ try -- любой OSError отсюда (директория,
доступ, ...) НЕ ловится этим кодом вовсе, пробрасывается ПРЕЖНИМ
ПОТОКОМ: наверх, где его либо ловит уже существующий тотальный try
main() (F-61 A1) -- одна строка stderr, rc 0, каталог НЕ ТРОНУТ
(карантин к этому пути даже не подошёл), либо (для dod_gate/main_gate,
чей A7-контракт НАМЕРЕННО без тотального try) пробрасывается ВООБЩЕ
наружу main() -- ожидаемое поведение по замыслу Р3а. (2)
`raw_bytes.decode("utf-8")` + `json.loads(...)` ВНУТРИ try -- ловит
ТОЛЬКО (UnicodeDecodeError, json.JSONDecodeError), т.е. содержимое
файла, который РЕАЛЬНО удалось прочитать байт-в-байт, но оно не
валидный UTF-8/JSON -- ЭТА ветка (и только она) квaрантинит, ровно
как до этой правки. Ветка "корень не-dict" (после успешного parse)
НЕ меняется -- всё ещё квaрантинит буквально как в Q503.
Не-цели: _quarantine_bad_track() САМА не меняется (она по-прежнему
глотает OSError -- это её собственный, отдельный от _load_track
fail-open периметр, живой ровно для сценария "карантин объективно
невозможен", НЕ для "мы решили не карантинить"); dod_gate_q503b.py/
main_gate_q503b.py несут ТОТ ЖЕ фикс, буквально идентичный код (K2).
===========================================================================
"""
...
def _quarantine_bad_track(path: Path) -> None:
    ...
    Q503B (t-531): эта функция НЕ ИЗМЕНЕНА -- её собственный fail-open
    периметр (глотает OSError изнутри) остаётся ровно для сценария
    "карантин объективно невозможен, ХОТЯ ВЫЗВАН ЗАКОННО" (напр. каталог
    недоступен на запись при подлинно битом СОДЕРЖИМОМ файла). Фикс
    Q503B -- НЕ ВЫЗЫВАТЬ эту функцию вовсе на "путь занят каталогом" --
    см. _load_track()."""
    try:
...
def _load_track(path: Path) -> dict:
    ...
    # -- пробрасывается ПРЕЖНИМ ПОТОКОМ наверх, где его ловит
    # существующий тотальный try main() (F-61 A1): одна строка stderr,
    # rc 0, каталог НЕ ТРОНУТ (карантин к этому пути даже не подошёл).
    # ДО этой правки path.read_text() был ВНУТРИ try/except ниже -- один
    # и тот же широкий except ловил и JSONDecodeError, и OSError
    # одинаково, что на "путь занят каталогом" уводило в
    # _quarantine_bad_track(), которая эмпирически УСПЕВАЛА
    # os.replace()-нуть каталог прочь (регресс против F-61 A8-пина
    # "каталог не должен исчезнуть/замениться файлом при отказе").
    raw_bytes = path.read_bytes()
    try:
        data = json.loads(raw_bytes.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError):
        # Q503 K1/Р4(а)+(в): нераспарсиваемое СОДЕРЖИМОЕ файла, который
        # РЕАЛЬНО удалось прочитать байт-в-байт (вкл. 0 байт) -- та же
        # КАРАНТИННАЯ ветка, что и до Q503B, просто уже сузана до
        # decode/parse-ошибок конкретно (не роняем хук, fail-open в
        # памяти НЕ меняется: edits/runs теряются ИЗ ЖИВОГО файла), но
        # байты не стираются молча -- уходят под карантинное имя
        # (_quarantine_bad_track).
...
def main() -> int:
    ...
    try:
        ...
        # A timeout here is caught by THIS SAME try/except boundary --
        # explicit early return matches the timed_out branch, not the
