...
def _is_continuation_path_token(tok: str) -> bool:
    ...
    слэш -- голый is_path_token() (используемый строкой МАРКЕРА, не
    тронут) засчитывает ЛЮБОЙ токен с "*" где угодно, включая markdown
    "**Внимание**:" -- см. докстринг модуля "ПЕРЕСДАЧА", п.2."""
    if not isinstance(tok, str) or not tok:
        return False
    if _WINDOWS_ABS_RE.match(tok) or _POSIX_ABS_RE.match(tok):
        return True
    return "*" in tok and ("/" in tok or "\\" in tok)


def _first_token_path(line: str):
    """Пересдача (не-блокер (а)): ПЕРВЫЙ пробельный токен строки
    продолжения (после снятия буллета `- `/`* `/`• ` ИЛИ номера списка
    `1.`/`2)`), если он валиден по _is_continuation_path_token --
    прозаический хвост после первого токена ИГНОРИРУЕТСЯ (не
    склеивается с путём через отсутствие запятой-разделителя). None --
