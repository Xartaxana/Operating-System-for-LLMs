...
# ---------------------------------------------------------------------
# C-К4 (2/4): длина в границах бюджета -- Р-С1 (числовой бюджет длины)
# НЕ НАЗНАЧЕН (docs/tasks/2026-08-25_warn-class-fix-dag.md:368-369,
# "Пробелы корзины" п.2: "Потолок длины additionalContext -- без него
# бюджет Р-С1 назначается вслепую"). Билдер НЕ изобретает число (Rule
# #1 кита: пробел, требующий проектного решения, возвращается вопросом,
# не решается уверенной догадкой) -- проверка длины ОСТАЁТСЯ НЕ
# РЕАЛИЗОВАННОЙ до ответа координатора. skip, не xfail: заглушка не
# должна маскироваться под "тест есть, но временно падает" -- она прямо
# говорит "проверки здесь ещё нет и почему".
# ---------------------------------------------------------------------


@pytest.mark.skip(
    reason="Р-С1 (числовой бюджет длины сообщения) не назначен спекой "
    "узла C -- возвращено вопросом координатору, не изобретено билдером "
    "(Rule #1). Реализовать длину-в-границах здесь после ответа."
)
def test_c4_length_within_budget_BLOCKED_on_R_S1():
    raise NotImplementedError("ждёт числового бюджета Р-С1")
