...
выше косметики отказа -- этот гейт обязан продолжать реально
БЛОКИРОВАТЬ (exit 2), fail-open-обёртка поверх ВСЕГО main() смазала бы
это). Ровно ОДНА правка против живого файла: _save_track() пишет трек
атомарно (mkstemp в той же папке + os.replace, суффикс .tmp
последний/не .json -- та же локальная схема, что dod_track_f61.py/
critic_snapshot_f61.py/dod_gate_f61.py, не общий модуль -- см.
...
def _atomic_write_text(path: Path, text: str) -> None:
    ...
    dod_gate_f61.py (не общий модуль, см. докстринг модуля, не-цели
    owns-спеки). Суффикс ".tmp" ПОСЛЕДНИЙ и НЕ ".json" --
