"""Fail-closed checker for the escape-allowlist (N4, docs/tasks/2026-07-21_validation-import.md,
mechanism #3): hash-pins CLAUDE.md's permanent escape/concession clauses to the
DECISIONS_FULL.md section that authorizes them, so a silent edit of the
underlying decision (the carrier text drifting away from its justification)
is caught mechanically instead of relying on someone noticing.
...
Usage:
    python tools/escape_check.py
        ...
        (each line names the entry id and the broken leg) otherwise.

    python tools/escape_check.py --hash D-XXXX
        Print the sha256 hex digest of decision section D-XXXX as found in
        docs/DECISIONS_FULL.md (the fixed decision carrier for this repo) and
...
"## D-00NN" or "## D-00NN -- title"):
    ...
    3. A line "opens" section <decision_id> when it matches
       ...
       "## D-0056b" does not either (extra letter) -- this is a superset of
       the literal spec requirement ("D-00561 must not match") applied
       symmetrically to any alphanumeric continuation, so an accidental
       near-miss id never silently matches.
    ...
    5. The section runs from that opening line (inclusive) up to but not
       ...
       end of file. ASSUMPTION (critic finding, N4 v3): a D-section's BODY
       never contains a line starting with "## " itself (e.g. a quoted code
       block reproducing another "## " heading verbatim); this generic
       boundary would truncate the section early if it did.
...
in substance?) -- it is NOT a text-integrity check; integrity of the cited
DECISION text is leg (c)'s job (the section hash). Carriers are reflowable
markdown (CLAUDE.md gets re-wrapped by boot-diet reflow); a byte-exact anchor
spanning a line-wrap point would break on every reflow with no substantive
rule change -- a false alarm by construction. Therefore, before the
substring containment check, BOTH the carrier's full decoded text and the
entry's carrier_anchor have every run of whitespace drawn from the set
...
def _ascii_safe(value):
    ...
    ASCII-safe here, confirmed by test_non_ascii_id_diagnostic_stays_ascii)
    -- guaranteeing the caller's output stays ASCII regardless of what a
    malformed/adversarial allowlist entry contains.
    """
...
def validate_entry_schema(entry, index):
    ...
    if "carrier_anchor" in entry:
        ...
        if not _is_nonempty_str(anchor):
            ...
            # critic finding #2 (N4 v3): a whitespace-only anchor still
            # passes _is_nonempty_str() (len > 0) but folds to "" / " ",
            # which is a substring of EVERY carrier text -- leg (a) would
            # then be vacuously true (a liveness check that can never
            # fail). Rejected at schema validation, before leg (a) ever
            # runs.
            errors.append(
