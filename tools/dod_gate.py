...
test_a8_path_occupied_by_directory[dod_gate] красная -- "путь занят
...
дод_gate ОБЯЗАН пробрасывать непредвиденные сбои, а не тихо
fail-open'иться -- enforcement выше косметики отказа, Р3а). ФИКС
(решение Lead, t-531), СИММЕТРИЧНО dod_track_q503b.py: ЧТЕНИЕ разделено
на ДВА шага -- (1) `path.read_bytes()` СНАРУЖИ try -- любой OSError
отсюда (директория, доступ, ...) НЕ ловится этим кодом, пробрасывается
ПРЕЖНИМ ПОТОКОМ ВООБЩЕ наружу main() (нет тотального try -- ожидаемое
поведение A7). (2) `raw_bytes.decode("utf-8")` + `json.loads(...)`
ВНУТРИ try -- ловит ТОЛЬКО (UnicodeDecodeError, json.JSONDecodeError):
СОДЕРЖИМОЕ файла, который РЕАЛЬНО удалось прочитать байт-в-байт, но
оно не валидный UTF-8/JSON -- ЭТА ветка (и только она) квaрантинит,
ровно как до этой правки. Ветка "корень не-dict" НЕ меняется.
_quarantine_bad_track() САМА не изменена (см. её докстринг).
===========================================================================
"""
