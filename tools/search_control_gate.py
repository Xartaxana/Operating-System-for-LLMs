...
rule 6: an empty content search is reportable only after a positive
control. The PreToolUse sibling (tools/hygiene_gate.py) catches
KNOWN-BAD invocations before they run; this one catches the case rule
6 actually names -- a search that came back empty -- and reminds at
the moment the misleading result appears, before it can be reported as
absence.

ORIGIN (D-0082 synk, 2026-07-29): this file is a PORT of the sibling
Dog deployment's tools/search_control_gate.py (their task t-021,
...
M). Semantics of the t-017 fix, the t-021 ledger addition, and the
D3/D4a attempt-2 fixes are preserved verbatim from the source; the
adaptations made FOR THIS DEPLOY are: (1) PowerShell -- this deploy
...
(2) this docstring, rewritten for this repo; (3) the CLAUDE.md rule
reference below points at this repo's own rule 6 (Command hygiene).
...
tools/search_control_gate_next.py (D-0069 -- a self-activating hook
file is never placed on its live path by its own builder); the Lead
moved it onto this live path and registered the PostToolUse hook in
...
search comes back empty) and a session ledger write (see t-021
...
written (source repo), only from a documentation summary. So the
...
t-017 FIX -- the probe was unreachable as designed, and a real class
of empty results was silently missed:
...
t-021 ADDITION -- session search ledger (half A of the negative-claim
correlation gate; see tools/claim_control_gate.py for half B):

WHY (source repo's own incident, carried verbatim): three times in one
session an agent asserted "X does not exist" without ever having
searched for X. CLAUDE.md rule 6 already forbids this; it was enforced
on discipline alone and discipline missed three times in one day. This
hook already observes every search as it happens (PostToolUse on
...
file); for Bash AND PowerShell (this deploy's adaptation -- see
...
this file's own t-017 fix already paid for once (a probe that never
...
fit_with_fixes, four Majors -- D2/D3/D4a are answered here; D1/D4b/D5/
D6/D7/D8 live entirely on the half-B side, see
tools/claim_control_gate.py's own docstring):
  ...
  file, attempt 1) read text via a bare `sys.stdin.read()` with NO
  ...
  `_extract_examine_term`. Rationale: rule 6 asks whether you LOOKED;
  reading a file is a positive control for that file's own existence,
  exactly as much as grepping for it. This does NOT make Read a
  general-purpose "search" for the empty-result warning above (Read is
  not added to `SEARCH_TOOLS`/`_looks_like_search` -- an empty file is
  not a miscall) -- it is purely an additional ledger source that half
  B's correlation can match against. See half B's own docstring for
  the residual this narrows but does not erase: a Read of file X
  defends a later claim ABOUT X's own existence, not claims about
  unrelated tokens X's content happens to mention.
"""
import json
import os
import sys
...
# F8 (критик t-339, прецедент tools/owns_gate.py's REGISTRY_COMPACT_
# THRESHOLD_LINES=500/_append_registry): ledger-файлы (и сессионные, и
# unknown.jsonl) append-only и НЕОГРАНИЧЕННО растут в долгоживущей
# сессии/репо. Компакция ПРИ ЗАПИСИ: если строк В ФАЙЛЕ (до добавления
# новой) СТРОГО БОЛЬШЕ порога -- сохраняется ТОЛЬКО ХВОСТ (последние
# LEDGER_COMPACT_THRESHOLD_LINES строк, самые СВЕЖИЕ) + новая запись;
# файл ПЕРЕЗАПИСЫВАЕТСЯ. В отличие от owns_gate.py (liveness-фильтр по
# TTL 24ч) здесь простое FIFO-усечение по ХВОСТУ -- ledger существует
# для КОРРЕЛЯЦИИ claim_control_gate.py против недавних
# поисков/чтений, свежие записи ценнее старых, TTL-семантики не нужно.
# ГРАНИЦА (правило 6а кита): РОВНО 500 существующих строк -> обычный
# append (500 НЕ строго больше 500); РОВНО 501 существующая строка ->
# компакция (501 строго больше 500) -- см.
# test_ledger_compaction_boundary_500_vs_501_lines в
# tools/test_search_control_gate.py. Применяется ОДИНАКОВО к
# сессионным файлам И к unknown.jsonl -- никакой отдельной пометки
# "unknown доминирует" НЕ добавлено (решение зафиксировано, в очередь
# по итогам обкатки).
...
# Tools that constitute a positive-control EXAMINATION (t-021 D4a) but are
...
# not a miscall. "select-string" added (this deploy's PowerShell
...
def _command_text_for_classification(tool_name, tool_input_raw):
    """F4(2) (критик t-339, живая репродукция): search-классификация
    ДОЛЖНА смотреть на саму КОМАНДНУЮ СТРОКУ (tool_input.command для
    Bash/PowerShell), а НЕ на JSON ВСЕГО tool_input. Раньше
    `_looks_like_search` получал `_as_text(tool_input_raw)`, который
    для dict-значения сериализует ВЕСЬ tool_input (включая, например,
    поле `description`) -- команда `git status --short` с description
    "Find modified files" ложно классифицировалась как поиск, потому
    что подстрока "find " (SEARCH_TOKENS) находилась в description, не
    в самой команде. ФИКС: для Bash/PowerShell берётся ИСКЛЮЧИТЕЛЬНО
    tool_input.command; для прочих тулов (кроме Grep/Glob, которые
    уже покрыты членством в SEARCH_TOOLS и вообще не нуждаются в этой
    функции) возвращает None -- не изобретаются другие текстовые поля."""
...
def _classify_result(result):
    ...
    before any JSON serialization -- see the module docstring's "t-017
    ...
    CONTENT/COUNT-MODE FP FIX (Lead live repro, 2026-07-29, confirmed
    empirically in a live session's own ledger -- see the delivery
    report for the reproduction): this hook falsely warned "Search
    returned nothing" on genuinely NON-empty Grep results in
    content-mode (returned the matching line text) and count-mode
    (returned "N total occurrences"), while an empty Grep/Glob still
    correctly warned and a non-empty Glob still correctly stayed
    silent. Root cause, confirmed by re-ordering the checks below: this
    harness's Grep dict response carries EMPTY `filenames`/`numFiles`
    in content/count mode (the file list is only populated in
    files_with_matches mode) even when `content` itself is genuinely
    non-empty -- the OLD check order tested filenames/numFiles BEFORE
    content, so a real match was misclassified "empty" purely because
    the (irrelevant, in this mode) file-list fields were empty. FIXED,
    conservatively (no `mode` key invented -- none was present in any
    observed payload; no schema sample was ever written for this case
    either, since the old code classified it as a definite "empty",
    not "missing"/"unjudgeable", so the probe-sample mechanism never
    even saw it): a non-empty string `content` now WINS over
    empty-looking `filenames`/`numFiles` -- checked and short-circuited
    BEFORE that branch, not after. An EMPTY (or absent) `content` falls
    through to the filenames/numFiles branch UNCHANGED, so a genuinely
    empty files_with_matches-mode Grep/Glob result still classifies as
...
def _session_id(payload):
    ...
    guessed aliases (module docstring, "t-021 ADDITION"). Returns the
...
def _extract_term(tool_name, tool_input_raw, command_fallback):
    """Search-term extraction for the ledger, per t-021 spec point 1:
    Grep/Glob take the pattern/path from tool_input; Bash AND
    PowerShell (this deploy's adaptation -- see module docstring
    "ORIGIN") take the WHOLE command string verbatim (no attempt to
    parse the term out -- half B does substring matching against it).
    Falls back to a stringified tool_input when nothing more specific
    is found, so a ledger line is never silently empty for an observed
    search."""
...
def _extract_examine_term(tool_input_raw):
    """t-021 D4a: the file path a Read call examined, checked under a
...
def _compact_ledger_lines(existing_lines):
    """F8: keeps only the TAIL -- the last LEDGER_COMPACT_THRESHOLD_LINES
...
def _append_ledger_line(payload, tool_name, term, is_empty, raw):
    ...
    search OR examination (t-021 D4a widened this from "search only").
    Wrapped by the caller in its own try/except -- see `_process` --
    so a ledger failure never affects exit code or the empty-search
    warning.

    F8 (critic t-339): compacts the ledger file AT WRITE TIME when it
    has grown past LEDGER_COMPACT_THRESHOLD_LINES -- see that
    constant's own docstring comment for the boundary contract (500 ->
    plain append; 501 -> compact-then-append) and the tail-keep
    rationale. Applies identically to a session-specific file AND to
    unknown.jsonl -- no special-casing between them."""
...
def _process(payload, raw):
    ...
    fail-open try/except (t-021 D3) -- no exception raised here can
    ...
    # F4(2): classification reads ONLY the command string (or SEARCH_TOOLS
    # membership), NOT the whole serialized tool_input (which `tool_input`
    # above still is, kept for _extract_term's own unrelated fallback use
    # below) -- see _command_text_for_classification's own docstring.
    ...
    # t-021: record every OBSERVED search to the session ledger, before
    # any of the existing sample/warn logic below. Wrapped so a ledger
    # failure never suppresses the empty-search warning.
    if is_search and tool_name:
        ...
        # t-021 D4a: a Read is a positive control for the read file's
        # own existence, exactly like a search -- recorded the same way.
        try:
    ...
    # t-017 widening: tool_name IS present and the tool/command looks
    # like a search, but the result is missing or of an unrecognized
    # shape -- this is exactly the case that used to pass silently
    # forever with no evidence ever landing (module docstring, "t-017
    # FIX" point 1).
...
def main():
    ...
    (t-021 D3, mirrors tools/hygiene_gate.py's and this delivery's own
