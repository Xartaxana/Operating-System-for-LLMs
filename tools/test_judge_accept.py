"""Snapshot from exam_hybrid_kit/tools/test_judge_accept.py, 2026-07-22
(D-0081 batch item е). Unit tests for tools/judge_accept.py -- mock HTTP
only, no network. Monkeypatches judge_client._post_chat_completion (the
module-level function judge_verdict() calls by default when its
_post_fn seam is not supplied -- production callers, including this
CLI, never pass _post_fn themselves, so patching the module function is
the only way to intercept the HTTP call from outside judge_client).
Adapted verbatim from the kit copy -- no changes needed beyond this
header (both modules under test now live alongside this file in this
repo's tools/, same import form as the kit).
...
def test_accept(tmp_path, monkeypatch, capsys):
    ...
    keys = _make_keys(tmp_path, ["- работает и продемонстрировано", "- нет крэша на пустом вводе"])
    fake_post, calls = _fake_post_factory('{"accept": true, "feedback": ""}')
    monkeypatch.setattr(judge_client, "_post_chat_completion", fake_post)

    rc = judge_accept.main(["--cell", str(cell), "--keys", str(keys), "--task", "Сделай мне календарь"])
...
def test_reject(tmp_path, monkeypatch, capsys):
    cell = _make_cell(tmp_path)
    keys = _make_keys(tmp_path, ["- ключ 1"])
    fake_post, calls = _fake_post_factory(
        '{"accept": false, "feedback": "нет обработки месяца 13"}'
    )
    monkeypatch.setattr(judge_client, "_post_chat_completion", fake_post)
    ...
    assert "месяца 13" in out["feedback"]
...
def test_keys_file_read(tmp_path, monkeypatch, capsys):
    ...
    cell = _make_cell(tmp_path)
    keys = _make_keys(tmp_path, ["первый ключ", "", "второй ключ  "])
    fake_post, calls = _fake_post_factory('{"accept": true, "feedback": ""}')
    monkeypatch.setattr(judge_client, "_post_chat_completion", fake_post)
    ...
    assert "- первый ключ" in prompt
    assert "- второй ключ" in prompt
...
def test_task_id_derived_from_keys_stem(tmp_path, monkeypatch, capsys):
    cell = _make_cell(tmp_path)
    keys = _make_keys(tmp_path, ["ключ"], name="tb")
    fake_post, calls = _fake_post_factory('{"accept": true, "feedback": ""}')
    monkeypatch.setattr(judge_client, "_post_chat_completion", fake_post)

    judge_accept.main(["--cell", str(cell), "--keys", str(keys), "--task", "текст узла"])

    prompt = calls[0]["prompt"]
    assert "Задача tb:" in prompt


def test_baseline_files_marker_present(tmp_path, monkeypatch, capsys):
    """baseline_files=None (spec: 'baseline-механики в песочнице нет --
    передавать baseline_files=None легально, маркер честен') must surface
    judge_client.BASELINE_UNAVAILABLE_MARKER in the material shown to the
    judge, not a silent unfiltered listing."""
    cell = _make_cell(tmp_path)
    keys = _make_keys(tmp_path, ["ключ"])
    fake_post, calls = _fake_post_factory('{"accept": true, "feedback": ""}')
    monkeypatch.setattr(judge_client, "_post_chat_completion", fake_post)
