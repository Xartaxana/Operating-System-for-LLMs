...
# ---------------------------------------------------------------------
# t-278 п.3: scratchpad-пути исключены из main-edit-скоупа целиком.
# ---------------------------------------------------------------------
    ...
    payload = {
        ...
        "tool_input": {
            "file_path": (
                ...
                "cea549ca-dcba-4cfd-98af-521ee41e34fe/scratchpad/script.py"
            )
        },
    }
    assert dod_track.build_fact(payload) is None
...
def test_build_fact_normal_repo_absolute_path_not_excluded(tmp_path):
    # Absolute path INSIDE the repo root (cwd) -- boundary against the
    # "outside repo root" branch: must NOT be excluded.
...
def test_build_fact_no_cwd_and_no_scratchpad_word_not_excluded():
    # Boundary: without cwd, the "outside repo root" branch cannot be
    # evaluated at all -- conservatively NOT excluded (fail-safe:
    # missing information does not earn an exemption).
...
def test_echo_json_scratchpad_edit_not_recorded_and_does_not_break_doc_only(tmp_path):
    """t-278 п.3: a coordinator writing a scratchpad temp script AFTER a
    doc-only edit must NOT be recorded to the track at all -- so
    main_gate.py's doc-only "whole-or-nothing" (t-278 п.1) still fires
    on what remains: only the genuine doc-only edit."""
    ...
    scratch_payload = {
        ...
        "tool_input": {
            "file_path": (
                "C:/Users/user/AppData/Local/Temp/claude/xyz/"
                "cea549ca-dcba-4cfd-98af-521ee41e34fe/scratchpad/tmp.py"
            )
        },
    }
