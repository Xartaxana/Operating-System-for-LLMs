...
def test_migration_adds_cache_token_columns(db):
    """A pre-t-075 database (has traffic_kind but no cache columns) gets
    the two nullable cache-token columns added on the next _connect(),
...
def test_success_row_fills_cache_tokens_from_usage(db):
    ...
    1.90.2 builds it from cache_creation_input_tokens /
    cache_read_input_tokens kwargs, verified empirically -- see
    sqlite_logger._cache_tokens) logs those counts into the new
    columns."""
...
def test_migration_adds_category_column(db):
    ...
    column (pre-t-085) gets the column added without losing existing rows."""
    from sqlite_logger import _connect
