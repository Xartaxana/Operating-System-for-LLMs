...
Projected-spend wall (t-324, external review P0, 2026-07-28): the
walls above only ever looked at ALREADY-LOGGED spend, so one large
request could cross a $ budget or a token quota mid-flight (nothing
projects the CURRENT request before the call). This adds a projection
layer that runs ONLY on the hook path, where the request payload
(`data`: messages/max_tokens) is available -- check_budget,
check_quota_windows and check_quota_pools all take an optional `data`
parameter; `data=None` is the pre-existing direct-call contract and
reproduces the old post-fact-only behavior byte-for-byte (see
...
`prompt_estimate` is ALWAYS computed (this is the main P0 driver -- a
single huge prompt gets caught even with a stock config): via
litellm.token_counter, falling back to a crude len(str(msg))//4
heuristic per message on ANY exception (unknown model, missing
tokenizer, malformed messages, ...) -- an estimation failure must
never fail the actual call. `output_allowance` is the request's own
`max_tokens` if a positive int; otherwise `projection.
assumed_max_output_tokens` from budgets.yaml (an int, or a
...
`quota_pools` (new top-level config key) group several aliases behind
...
Known asymmetry (critic review, class F-38 / SIBLING_MAP axis 2,
2026-07-28): the projection's INPUT is only the request's message
...
report INCLUDING cache_read_input_tokens / cache_creation_input_tokens
...
def load_budgets() -> dict:
    ...
    # "projection" is deliberately NOT defaulted to {} here (t-324):
    # its ABSENCE vs. presence is itself the byte-for-byte compatibility
    # gate for the $ projection wall (see check_budget below) --
    # a config that never mentions "projection" must never engage any
    # of the new $ price-lookup/fail-closed/warn-no-price machinery,
    # even though the request payload (data) is always available on the
    # hook path. config.get("projection") is None for such a config;
    # code below checks `is not None`, not truthiness, on purpose.
    return config
...
def quota_pools_containing(config: dict, model: str) -> list:
    ...
    `aliases` list includes `model` (t-324, D-0058 P0 fix: several
    aliases sharing one provider key share one physical quota ceiling).
    A model in no pool is a no-op, same "absent means no wall"
    convention as quota_windows_for."""
    return [
        (name, pool)
...
def tokens_in_window_for_aliases(conn: sqlite3.Connection, aliases: list, since_iso: str):
    ...
    aliases at once (t-324 quota_pools: several gateway aliases can
    share one provider key's physical quota, so the pool wall must sum
    their combined spend, not check each alias in isolation). Same
    NULL-safe COALESCE and all-traffic-kinds semantics. An empty
    `aliases` list is a no-op (0, None), never a malformed "IN ()" SQL
    clause."""
    if not aliases:
        return 0, None
...
# --- Projected-spend estimation (t-324, external review P0) --------------
...
def _coerce_int(value):
    ...
    not ALREADY a native int/float (t-324 critic fix #1, 2026-07-28: a
    quoted scalar in YAML, e.g. `assumed_max_output_tokens: "2048"`,
    parses as a Python str -- deliberately rejected here even though
    Python's own int("2048") would happily succeed, because a quoted
    number in a numeric YAML field is almost always an operator typo
    that deserves a warning, not silent acceptance). `bool` is excluded
    (True/False are technically int in Python but never a meaningful
    token count). The caller treats None exactly like "absent" (0),
    the same fail-safe direction as every other estimation failure in
    this module: a malformed config value must never fail the actual
    call."""
...
def _bad_assumed_output_warned_today(conn, model, today) -> bool:
    ...
    (t-324 critic fix #1), sibling of _no_price_warned_today above.
    Reuses budget_events (spent_usd/budget_usd recorded as 0.0 -- this
    warning is not a $ figure, but the schema's NOT NULL columns need a
    value and 0.0 is an honest sentinel, not a guessed cost)."""
    row = conn.execute(
        "SELECT 1 FROM budget_events"
...
def warn_if_assumed_output_malformed(conn, model: str, today: str, config: dict) -> None:
    ...
    (t-324 critic fix #1) -- output_allowance() already fails safe (0)
    on its own, this only makes the misconfiguration VISIBLE instead of
    silently degrading the wall."""
...
def check_budget(model: str, data: dict = None) -> None:
    ...
    exhausted by the current request (t-324, external review P0).

    `data` (the request payload: messages/max_tokens) is optional and
    defaults to None: `data=None` reproduces the pre-t-324 direct-call
    contract byte-for-byte -- no projection code below ever runs, only
    the original post-fact wall. Guard.async_pre_call_hook always
    passes the real payload; the projection layer only exists on that
    path, and even then is gated on `projection` actually being
    configured (module docstring)."""
    config = load_budgets()
    ...
    try:
        ...
        if data is not None:
            # t-324 critic fix #1: fire this BEFORE any projection math
            # runs -- a malformed assumed_max_output_tokens must be
            # visible even on a $ wall that has no usd_prices entry at
            # all (output_allowance's own fail-safe already prevents a
            ...
            if projection is not None:
                ...
                if price is not None:
                    ...
                    if model in fail_closed:
                        # t-324 critic fix #2: a fail-closed refusal must
                        # leave a trace in budget_events (level
                        # "block_no_price") -- previously this branch
                        # raised with NO row at all, making unpriced
                        # fail-closed refusals invisible to the Ledger
                        # digest. spent/budget are real, known figures;
...
def check_quota_windows(model: str, data: dict = None) -> None:
    ...
    (t-324, external review P0). A model with no configured windows is a
    no-op, same convention as check_budget.

    `data` is optional and defaults to None: `data=None` reproduces the
    pre-t-324 direct-call contract byte-for-byte (post-fact wall only,
    no projection). Guard.async_pre_call_hook always passes the real
    payload; unlike the $ wall, the token projection here does NOT
    ...
    `projected_tokens` (t-324 critic fix #3) is computed ONCE per call,
    before the loop below -- an alias can carry several windows (e.g.
    a 60s TPM wall and an 86400s TPD wall), and recomputing it per
    window would run litellm.token_counter that many times on every
    single pre-call request for no benefit (the projection value does
    not depend on which window is being checked)."""
    config = load_budgets()
...
def check_quota_pools(model: str, data: dict = None) -> None:
    ...
    (t-324, external review P0 recommendation #5: several aliases can
    share one provider key's physical quota ceiling) is exhausted --
    post-fact (spend already over the wall, summed across ALL aliases
    in the pool) or PROJECTED to be exceeded by the current request. A
    model in no pool is a no-op, same convention as check_quota_windows.
    ...
    pre-t-324 legacy behavior to preserve either way.

    `projected_tokens` (t-324 critic fix #3) is computed ONCE per call,
    before the pools/windows loop below, for the same reason as
    ...
    debounce as an alias's own quota_windows (t-324 critic fix #4,
    Rule #1: cheap, and the class should behave the same way)."""
    config = load_budgets()
...
class Guard(CustomLogger):
    async def async_pre_call_hook(self, user_api_key_dict, cache, data, call_type):
        ...
        # Order (t-324 spec): $ budget -> per-alias token windows -> shared
        # pools; a per-alias wall blocks before the pool-level one runs.
        # `data` is always passed here (never None) -- this is the ONLY
        # call site where the projection layer engages; direct calls
        # (e.g. from tests or scripts) that omit `data` get the
        # pre-t-324 post-fact-only contract.
