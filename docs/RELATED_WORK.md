...
## Harness engineering (Лопополо/OpenAI) и система Case (2026-07-13, слово оператора)

Источники: Ryan Lopopolo, «Harness engineering: leveraging Codex in
an agent-first world» (OpenAI, 2026-02-11,
https://openai.com/index/harness-engineering/ — оригинал отдаёт 403
для fetch-тула; читано по двум независимым зеркалам:
https://jaytaylor.com/notes/node/1770842156000.html и
https://github.com/celesteanders/harness/blob/main/docs/research/260211_openai_harness_engineering_codex.md)
+ доклад о системе Case — личном harness'е автора доклада,
реализующем идеи эссе (субтитры эпизодов 3, 9–13 от оператора,
D:\Improving_AI\субтитры.txt). След прохода Lead (D-0066): субтитры
прочитаны дословно; эссе — двумя независимыми извлечениями с
кросс-сверкой несущих утверждений по обоим зеркалам, включая
негатив «экономики ярусов в эссе нет».

Кредо школы совпадает с нашим дословно: «replace your trust with
evidence», «enforce with code, not prompts», «every failure is a
harness bug» — D-0052/D-0046, D-0063 и D-0043 записываются
НЕЗАВИСИМОЙ СХОДИМОСТЬЮ с этой школой, не заимствованием. Масштаб
их данных: 5 месяцев, ~1 млн строк, ~1500 PR, 0 строк рукописного
кода («Humans steer. Agents execute»).

Взято (2026-07-13, приказ оператора «делаем всё предложенное»):

1. **Execution plans как first-class артефакты репо** («сложная
   работа — в execution-планах с progress- и decision-логами,
   закоммиченными в репозиторий; активные, завершённые планы и
   техдолг версионируются рядом») — прямой внешний приор для
   workstream 3 (D-0059): пилот носителя task-графа (docs/tasks/)
   делает ровно это. Приор пишется в актив пилота для N5-разбора.
2. **Ретроспективный агент Case + pattern capture Лопополо**
   (ретро-агент читает транскрипты: doom-loops, повторные вызовы;
   «review-комменты и баги кодируются в доки и тулинг») →
   ретро-контур калибровки: чеки 0/19–24 протокола + бэклог
   docs/RETRO_PATTERNS.md (порядок важности — слово оператора).
3. **Не-кодовый witness для UI-работы** (Case: Playwright-видео
   «до/после» прикладывается к PR; код не смотрят, пока результат
   не доказан не-кодом) — расширение типологии witness D-0052 для
   UI-задач; очередь CURRENT_CONTEXT, триггер — первый UI-диспатч
   возобновлённого AO3-конвейера.
4. **A/B-eval механизма качества** (Case: скилл давал 77% против
   97% без него — автор узнал только замером) — принцип в eval-план
   stage 2 и бэклог ретро-паттернов (№12): механизм с претензией на
   улучшение качества получает малый замер «с ним vs без».
5. **«Guide, don't prescribe» / gotcha-стиль** («не резюме всех
   доков, а условные правила вокруг того, что агенты стабильно
   делают не так») — для onboarding-доков тулкита: копить
   gotcha-правила из пользовательских установок (F-35/F-36 —
   первые два), не расширять README; порт-очередь D-0074.
6. **Doc-gardening → механическая валидация свежести** (рекуррентный
   агент ищет протухшие доки, CI валидирует базу знаний) — заказ
   оператора по живому прецеденту (протухший ROADMAP, F-41):
   чек 24 калибровки, еженедельно.

Не строим (Rule #1, чтобы не ре-литигировать): state machine поверх
Claude Code сессий (наша политика-как-роутер; промоция в код — по
evidence, D-0063; для AO3-конвейера state machine уже есть —
rules.yaml), memory-файлы вне git (git уже память, D-0001/D-0014),
quality grades + автоматический debt-paydown PR'ами (масштаб не
наш), агент-легибельная observability (CDP/LogQL/PromQL — кандидат
AO3 на evidence QA-узкого места, не сейчас).

Находка позиционирования (для White Paper related work): в эссе НЕТ
слоя экономики — ни ярусов моделей, ни маршрутизации
дешёвый/сильный, ни токен-бюджетов, ни стоимости (негатив сверен по
двум зеркалам). Harness engineering решает надёжность; наша система
...
## agent1st (2026-08-10, запрос оператора; вердикт «не берём»)

https://github.com/applerom/agent1st — минималистичный стартовый
протокол «разработчик ↔ агент» (AGENTS.md, ~200 строк, 11 принципов:
role contract, done-is-not-a-mood, right to disagree, attention
engineering, semantic hygiene, CDD, agent loop, weak-signal,
delegation design, semantic logging, continuity). Целевая аудитория —
проекты, начинающие с нуля.

Вердикт (слово оператора 2026-08-10): не берём. Каждый из 11
принципов у нас покрыт более сильной И обеспеченной формой (хуки,
валидатор журнала, детекторы калибровки): witness D-0052/D-0053,
DoD-чеклист R11, гигиена п.6 (F-30/F-34), журнал типизированных
событий, CURRENT_CONTEXT/handoff. По нашему же стандарту D-0065
agent1st целиком декларативен — ни одного механизма исполнения.
Единственный кандидат-находка — форма жалобы «Проблема → Влияние →
Минимальный фикс» — отклонён: конфликтует с R9 (чини класс, не
инстанс), функция уже у failure_class в rejected. Второй проход
D-0066 не требуется при вердикте «не берём»; при возврате к теме —
прицельный проход по дословному тексту (первый проход — WebFetch-
пересказ малой моделью raw AGENTS.md + README, трейл в журнале,
dispatch_skipped 2026-08-10T22:08:20).
