...
- [2026-07-24 внешнее ревью Codex](2026-07-24_codex_external_review.md) — сторонний read-only ревью policy-as-code/делегирования/экономики (2×P0, 5×P1, 2×P2, 10 продуктовых предложений); файл оставлен незакоммиченным сессией 07-24 (пропуск handoff, D-0050 — материал чека №5), закоммичен триажем 07-28
- [2026-07-28 триаж ревью Codex](2026-07-28_codex-review-triage.md) — вердикты Lead по всем находкам: оба P0 подтверждены спот-проверкой кода (Guard без проекции расхода; Bash у scout) и приняты в работу, 3 кандидата калибровки №5, 3 отклонения с reopen-триггерами, остальное покрыто D-0080/D-0090/D-0091/D-0093
- [2026-08-05 калибровка №6: закрытия дня](2026-08-05_calibration6-closures.md) — VERBATIM закрытых блоков, снятых диетой 08-05: находки калибровки №6 (R3-обход денежного диффа, молчащий critic_snapshot, многострочный owns-блок, отклонение t-349, носитель судьи, рост живых файлов), батч мелочей t-353..t-355 с критик-гейтом (2 блокера, оба закрыты пересдачами; канон 2068 passed), находка «неизвестный писатель token_usage.xlsx» (закрыта ПО КЛАССУ, личность не установлена), хвост (е) D-0099 и история обкатки owns_gate/given-path 07-28..07-30; живые остатки (подкласс owns №2, льгота skip, неэффективность warn гигиены, кросс-пункты AO3, подкласс R4 и класс «ярус ≠ subagent_type») оставлены указателями в очереди
- [2026-07-30 boot-diet: развёртка CURRENT_CONTEXT после релиза v0.6.0](2026-07-30_boot-diet-relocations.md) — VERBATIM девяти закрытых блоков, снятых диетой 07-30 (нарратив Phase 4 до закрытия подписью; входящее AO3 07-24; Dog-разбор 07-29 закрытая часть; кросс-пункт AO3 07-28; ревью Codex исполненная часть + сводка порт-очереди релиза v0.6.0; батч мелочей 07-28; итог-абзац калибровки №5; онбординг D-0090; пилот opus-дизайнера с вердиктом промоции); живые остатки (обкатки owns_gate/given-path/корреляции, очередь передачи Dog, named-узлы минора, хвосты Phase 4) оставлены на месте
- [2026-08-16 boot-diet: развёртка после пробоя 147 КБ](2026-08-16_boot-diet-extractions.md) — VERBATIM шести блоков, снятых диетой 08-16 при пробое 147151/100000: прежний список неисполненного находок калибровки №6 (все пункты сверены исполненными по ядру CLAUDE.md, кроме подкласса owns №2), прежний текст обеих веток порта правила 9 journal_validator, разбор отзыва находки «раскол экзаменов» с причиной ошибки и полным обоснованием живого пункта про пустой проект, оба входящих AO3_tests 08-15 (binding-aware чеки + детектор heartbeat-fastdeath, исполнены 08-16), входящие AO3 08-09/08-14 (признанные отличия их словаря, класс basis=judge неприменим у них), входящее Dog 14.08 (D-0099 у них исполнен, worker_ref принят, их батч v0.8.1 принят). Живые остатки перечислены в конце файла списком из десяти пунктов и оставлены строками в очереди; CURRENT_CONTEXT срезан 70544 -> ~26 КБ
- [2026-08-27 POLICY_FULL N3-снимок: вынесенный архив](2026-08-27_policy-full-n3-snapshot.md) — диета t-652 двух корпус-файлов: раздел 1 — ДОСЛОВНЫЙ остаток снимка CLAUDE.md N3-среза (2026-07-19) из docs/POLICY_FULL.md минус два живых амендмента (11(q)/D-0106, п.6(а)/D-0105), поднятых ##-секциями в POLICY_FULL.md; раздел 2 — остаток нарратива блока «Диета ядра 2026-08-16» (вступление + «Что осознанно не тронуто» + «Диета W2c-1»), не вошедший в промоцию 13 ###-подсекций до ## тем же ходом
