...
## Приложение: N4 — первый честный срез R2/R3 (2026-07-13 вечер, досрочно по слову оператора)

Узел N4 пилота (docs/tasks/2026-07-13_calibration-week.md) исполнен
ДОСРОЧНО: данные окна заморожены (прокси опущен, real-строки не
прибывают), ожидание 07-18 им ничего не добавляло. Окно среза:
real-строки Claude-passthrough алиасов с 2026-07-12T22:15 — 874
строки, $174.45.

**R2 — ЧЕСТНЫЙ ВЕРДИКТ: НЕ ИЗМЕРИМ на turn-shaped данных; обе
защитимые оценки далеки от порога с разных сторон.**
Штатный readiness (t-090) печатает R2=100% met (coding
$174.78/174.88) — это артефакт, предсказанный критиком t-090:
keyword-эвристика видит код в контексте и лепит coding на ВСЁ.
Аудит выборки (топ-4 по цене + случайные): «coding»-строки — это
ЦЕЛЫЕ ХОДЫ координаторских Claude Code сессий («как идёт прогон?»,
task-notifications, suggestion-mode харнесса) — ходы Лида, не
задачи кодогенерации. Структурная причина: real-строка прокси =
ХОД сессии, строка таблицы делегирования = ЗАДАЧА; turn != task,
классификатор задач на ходах не имеет предмета. Честные оценки:
(а) раскладка по МОДЕЛЯМ (честна по построению): Fable
(Lead-ходы) $140.07 = 80.3%; делегированные ярусы (opus=critic
$27.25, sonnet-4-6=builder $5.15, sonnet-5 $1.27, haiku=scout
$1.15) = ~$34.8 = 19.7% — делегируемая работа УЖЕ извлечена
routing-политикой в субагентные вызовы, остаток Lead-ходов —
координация по построению; (б) validated-delegable БЕЗ coding
(кандидат-вердикт rejected, 4 прогона + 2 кандидата ниже порога):
formatting $0.02 = 0.01% << 25%. При ЛЮБОМ исходе строки coding
R2 порога не достигает: либо категория rejected (и её $ не
считаются), либо считается всё (артефакт).

**R3 — формально 0.1пп сдвига (coding 100.0% -> 99.9% между
половинами окна, split 2026-07-13T01:19) — «met» тривиально из-за
вырожденности классификатора; информативности нет.**

**Следствие для Router-гейта: решение «оставить ЗАКРЫТЫМ»
подтверждено срезом с запасом.** R1 набран (31/6), но R2
недостижим на текущем real-трафике; честное R2 потребует
task-shaped учёта (например, per-dispatch аккаунтинг субагентных
вызовов — уже есть в cc_usage agent-атрибуции) — вопрос к N5/
калибровке, не к постройке. Урок для White Paper: «turn != task»
— классификатор категорий применим к диспатчам и synthetic-
прогонам (где есть ground-truth, t-085), не к органическим ходам.

Метод: gateway/metrics.py --days 2 (readiness t-090) + ручные
SQL-срезы (границы окна, половины, аудит 6 строк id 1062/940/
1326/1501/801/1112 с извлечением последнего user-сообщения).
Статусов таблицы срез НЕ двигал (Update Rule 1 — калибровка).
