...
- CLAUDE.md DEEP DIET: две точки ядра — №15 0.94 (−44% цены), №16
  0.90 (планка 0.93 НЕ взята; съедена формой: журналы 11 дефектов,
  t2-план без файла — не ядро-специфично содержательно) → N3-срез
  СТОИТ по предрегистрации, доклад оператору сдан (варианты: третья
  точка №17 / срез его словом / не резать). D-0083 введён (tier_echo
  на SubagentStop; канал hookSpecificOutput после k6-замера); подмен
  прокси в №16 нет. DAG: docs/tasks/2026-07-19_claude-md-diet.md.
