...
- ВАЛИДАЦИОННЫЙ СЛОЙ: DAG docs/tasks/2026-07-21_validation-import.md
  (N1+N2+N3 done — промоушен critic.md 07-22 по экзамену №2 t-260:
  два прогона opus, PASS по валидным ключам, rule 16 активен —
  координатор гоняет tools/critic_verdict_check.py на каждом
  критик-вердикте до приёмки; N4 queued). ОЧЕРЕДЬ КАЛИБРОВКЕ №4:
...
- ТУЛКИТ: релизы v0.4.0–v0.4.2 вышли 07-20, батчи 1–3 исполнены,
  ...
  session-handoff/калибровочного протокола); мораторий до слова.
