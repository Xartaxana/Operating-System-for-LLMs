...
- СЛЕДУЮЩАЯ СЕССИЯ — ДВА ПУНКТА ПО СЛОВУ ОПЕРАТОРА 07-20 («пятого
  раза достаточно, не ждать неделю»), приоритет над очередью №4:
  (1) ПОЧИНКА КЛАССА «финал-сообщение воркера теряет содержательную
  часть»: 5 прецедентов за 07-20 (t-226, t-225, t-231, t-234 —
  дозапросы «повтори отчёт»; паттерн: после Stop-hook-пересдачи
  witness воркер шлёт ТОЛЬКО последний turn, считая прежний текст
  доставленным; координатору предыдущие сообщения НЕ доставляются).
  Направление фикса (механизм, полный rule-10): правило в
  role-файлы builder/scout/critic «финальный отчёт — полным текстом
  в КАЖДОМ завершающем сообщении; пересдача witness = повторить
  отчёт целиком, ссылка на предыдущее сообщение запрещена» +
  проверить текст-подсказку dod_gate на пересдаче; сиблинги — кит
  (ось 7, очередь порта) и AO3-агенты (ось 1/3). Смежный пункт
  очереди №4 «пустые финал-сообщения воркеров ~200K+» — тот же
  класс, закрыть вместе.
  (2) ПРОВЕРКА ВТОРОГО РОУТЕР-КАНДИДАТА на стенде labeled_v2 (21
  пункт, песочница routellm_smoke): (а) causal_llm (8B, локальный,
  тот же оффлайн-метод, CPU); (б) haiku-классификатор промптов
  (API, ~копейки на решение) — тем же AUC-протоколом, ключи те же
  (полный + экзамен-срез); вердикты в план оценки
  (docs/tasks/2026-07-20_routellm-evaluation-plan.md). bert уже
  отклонён evidence'ом (AUC 0.60 на экзамен-срезе, коммит 20259ff).
...
- CLAUDE.md DEEP DIET ЗАВЕРШЁН: N3-СРЕЗ ИСПОЛНЕН 07-19 (слово «да
  ...
  Очередь калибровке №4: rejected-на-критик-отказ (кандидат чека);
  пустые финал-сообщения воркеров ~200K+ токенов; синтетика окон
  №15–№17 (D-0075); не-блокеры критика t-208 (глоб → слаг репо;
  multi-tier model). DAG: docs/tasks/2026-07-19_claude-md-diet.md.
...
- НОЧЬ 07-20 «ПРИВЯЗКИ API-КОНТУРА» (слово оператора; t-219..t-224
  ...
  «Two Vocabularies» (ось 4/7 D-0085). ОНБОРДИНГ-ЭКЗАМЕН API-КОНТУРА
  ОСТАЁТСЯ (слово оператора 07-20): модели ПОЛЬЗОВАТЕЛЯ на его
  ключах экзаменуются против референс-планок D-0085 «не хуже
  зеркала» (судья — калибровка 13 пар с планкой 12/13; builder —
  реплей-корпуса как ночные; критик — CRITIC_EXAM D-0071; scout —
  golden set); кит-батч несёт планки числом, не только процедуру.
