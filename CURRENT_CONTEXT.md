...
- НАБОР №3 = ДИЕТ-B, ИТЕРАЦИИ 1–2 ЗАКРЫТЫ 07-16 (прогоны №7/№8/№9;
  разбор целиком — docs/tasks/2026-07-16_economy-exam-run7.md, 3
  строки Runs log). Тренд 5 точек: Bfull 0.93 ($10.33) / v1 0.91 /
  v2 0.85 / v3 0.65 ($13.41) / C 0.71 — чем жёстче урезан критик,
  тем хуже и дороже. РЕШЕНИЕ РАЗБОРА: ДИЕТА КРИТИКА ЗАКРЫТА
  (per-diff критик возвращается); ИЗ ДИЕТЫ ОСТАЮТСЯ как честность,
  не экономия: батч-журнал, validator standalone (в штабе, b5a33c0),
  dispatch_gate, dod_gate+критик-снимок — kit v3-final @ 61f3c8d,
  весь policy-as-code стек жив и доказал себя измерителем. ПРОГОНЫ
  №10/№10б («текущее+гейты», гипотеза оператора): дисперсия ×2.6
  доминирует — решает медиана калибровочных малых экзаменов;
  устойчиво: 0 ложных блоков за 4 прогона, снимок/валидатор/форензик
  работают, main-сессия — единственная слепота (4-е подтверждение).
  ОЧЕРЕДЬ policy-as-code (v5): Stop(main)-гейт; VERIFICATION_
  COMMAND_RE без самотестов гейт-инфры; чек «kit-сборка диффом
  против эталона» (defect_found t-155). Порт гейтов в штаб/тулкит —
  слово оператора. Большой прогон — по каденции. РЕЗЕРВ — генератор
  сайта.
