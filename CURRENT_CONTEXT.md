...
- CLAUDE.md DEEP DIET: ТРИ точки ядра — 0.94/0.90/0.92, медиана
  0.92 (планка 0.93 не взята на 0.01) → N3 ждёт слова оператора.
  Доработки журнальной дисциплины СРАБОТАЛИ: k2 11→0 (ядро v3
  таблица форм + хук journal_echo на записи, активен в штабе).
  D-0083: tier_echo СНЯТ с пути (SubagentStop кормит субагента —
  defect_found t-203; доставка в редизайне, кандидат Stop-хук);
  2-й замеренный экземпляр класса — частичная mid-worker подмена
  (fable→sonnet 2/11 ходов, №17-t2). Подозреваемый ядро-класса
  один: k4 inline-DoD тренд вниз (R11 сжат) — кандидат точечной
  строки. DAG: docs/tasks/2026-07-19_claude-md-diet.md.
