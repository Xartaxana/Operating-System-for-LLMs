...
крупнейший кусок, порядок стройки 1–7). Прогресс стадии 4 (2026-07-11): пп.1-5 ПРИНЯТЫ — политика+роли EN
(t-045, critic), скелет (t-046), скиллы EN вкл. НОВЫЕ
onboarding/scout-exam-gen (t-047/t-049; override-событие = rejected,
решение Lead), PROCESS EN с reserved-чеками 15/17 (t-048), долг N2
закрыт (чеки поименованы в политике шаблона). ЖДЁТ: t-050 (перенос
tools/gateway) → сборочный critic → ПЕРВЫЙ ПУШ в
Supervised-Delegation. Стадия 5 ПОДТВЕРЖДЕНА оператором: 5а прогон
онбординга на пустом проекте (нами), 5б установка в ТРЕТИЙ проект
оператора (путь уточнит при установке) → после обоих оператор
раздаёт желающим (снятие pre-release = стадия 6 после фиксов). ПРИОРИТЕТ ОПЕРАТОРА
2026-07-11: закрытие Phase 2 — ПОСЛЕ первого пуша тулкита;
текстовые задачи для API-builder (builder-groq) забираются по ходу
стройки тулкита. ОТКРЫТО У ОПЕРАТОРА: подтверждение MIT (В4 «да»
без текста лицензии). Phase 1/1.5 ЗАКРЫТЫ 2026-07-11 (подпись
...
очереди — on-touch/evidence-gated. НОВОЕ в очередь: фикс
дайджеста (defect_found ref=t-002, журнал 14:35; id выдаст диспатч) —
metrics.py R1 читает Shadow-лог по старому пути (пропущенный
потребитель переноса D-0067) + захардкоженный env-негатив в
R5-тексте; мелкий builder-фикс на следующем касании gateway/ или
отдельной задачей.
