...
- БАТЧ МЕЛОЧЕЙ 07-20 ИСПОЛНЕН ЦЕЛИКОМ 07-22 (t-261, все 7 пунктов;
  ...
  docs/task_reports/2026-07-22_night-validation-closures.md.
- РЕЛИЗ КИТА v0.5.0 ОТЛОЖЕН (слово оператора 07-22 «сначала
  докажем что наш подход работает (на калибровке), потом будем
  делать релиз»): релизный снимок staging → публичный
  Supervised-Delegation стоит ЗА ГЕЙТОМ калибровки №4 (~07-25) —
  чистый аудит чека 30 (judge-окно дня: 8 листов, 12 reject'ов,
  4 эскалации, 1 failed-back, 2 spec-дефекта DoD, отклонения формы
  судей) + чек 31 + экономика. Staging готов (R13, D-0090-онбординг,
  witness-слой, гейт-батч) и заморожен до слова после аудита.
...
- БАТЧ МЕЛОЧЕЙ (к следующему builder-батчу, рекомендации критика
  t-278, не-блокеры): (а) докстринг find_tier_declarations —
  анти-спуфинг-гарантия переоценена (одностроковый спуфер проходит
  обе семантики; реальный эффект — fail-closed на цитатах);
  (б) подстрока 'scratchpad' в _is_scratchpad_path избыточна к
  вне-cwd условию и даёт латентный fail-open на гипотетическом
  репо-файле с таким именем — рассмотреть сужение; рамка вне-cwd
  исключения (per-repo гейтинг) принята координатором, зафиксирована
  этой строкой; (в) session_context: returncode git ls-files не
  проверяется — при ошибке git с пустым stdout хуки метятся
  «untracked» вместо честной причины (критик t-288, не-блокер);
  (г) явный тест «skip-строка ПЕРВОЙ строкой сообщения» (эмпирика
  критика t-288: True; покрыть кейсом).
