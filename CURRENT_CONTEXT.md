...
- **НАХОДКИ КАЛИБРОВКИ №6 (08-05) — ЖИВОЙ ОСТАТОК.** Закрытые пункты
  ...
  (10) **Кросс-пункты AO3 ПЕРЕДАНЫ 08-05** (слово оператора «в АО3
  можно коммитить»; их дерево было чистым и запушенным — сверено ДО
  правки, D-0060): блок «ВХОДЯЩЕЕ ИЗ OS 2026-08-05» в их
  docs/HANDOFF.md, их коммит ff956d8, запушен. Передано (а)(б)(в) +
  ответ (г) на их кросс-пункт 08-02 п.1 + напоминание (д) про
  открытое решение по D-0099. РЕШЕНИЯ ЗА НИМИ; отследить ответы при
  следующей сверке. Исходный состав пунктов:
  ...
  (13а) **ТВИН ПРАВИЛА ОТКАТА В РОЛЬ-ФАЙЛАХ — ЗА ЭКЗАМЕНОМ КРИТИКА.**
  Норма п.7 командной гигиены внесена в ядро 08-05 и действует для
  ВСЕХ ярусов (класс шире критика: мутационные пробы гоняют и builder,
  и сам Lead). Твин в .claude/agents/critic.md и builder.md НЕ внесён
  сознательно: правило 10 самого critic.md требует прогона
  PROCESS/CRITIC_EXAM.md ДО коммита правки роль-файла, а его же
  триггер п.1 запрещает in-session прогон правки роли (кэш
  определений агентов). Гейт не скиплю. Твин уезжает СЛЕДУЮЩИМ
  прогоном экзамена — либо по слову оператора отдельным ходом из
  свежей сессии; до тех пор норма держится ядром, которое роль-файлы
  и так наследуют.
