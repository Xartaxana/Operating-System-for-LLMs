...
CURRENT TASK СЛЕДУЮЩЕЙ СЕССИИ (решение оператора): стадия 5а —
прогон онбординга на ПУСТОМ проекте (свежая сессия-«незнакомец»:
clone шаблона → README → INSTALL → onboarding-скилл до первого
делегированного цикла; все спотыкания = находки в фиксы шаблона);
затем 5б — установка в ТРЕТИЙ проект оператора (путь спросить у
оператора при старте). После обоих оператор раздаёт шаблон желающим;
снятие pre-release = стадия 6. ПРИОРИТЕТ ОПЕРАТОРА
