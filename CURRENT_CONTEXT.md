...
- НАБОР №2 ЗАКРЫТ 07-16 (финал: №5 0.64/0.70/0.70, №6 0.72/0.86/
  ...
  + Runs log. ОСТАТОК: фиксы — validator no-op вне git + пустой
  stdout при rc=0 без пометки; калибровке — синтетика окон (D-0075),
  счётчик возвратов воркеров, «Sonnet не держит дисциплину текстом»
  в D-0058-разбор.
- НАБОР №3 = ДИЕТ-B, ИТЕРАЦИЯ 1 ЗАКРЫТА 07-16 ночью (прогоны №7
  Bfull/Bdiet-v1/C + №8 Bdiet-v2 с критик-гейтом кодом; разбор обоих
  — docs/tasks/2026-07-16_economy-exam-run7.md, 2 строки Runs log).
  Векторные скаляры: Bfull 0.93 ($10.33) / v1 0.91 ($11.33) / v2
  0.85 ($12.21) / C 0.71 ($7.13). ВЫВОДЫ: (1) механизация каденции
  РАБОТАЕТ — хук удержал 1 критик-вызов, Sonnet текстом не
  управляется, кодом управляется (D-0063 на координаторе; актив в
  kit v2 @ c3c267f); (2) диета критика НЕ окупилась — full лучший
  при меньшей цене, per-diff критик дёшево ловил реальные дефекты;
  резать стиль координатора (батч/балк/main-ходы 36→10) — да,
  каденцию критика — нет; (3) вектор ловит то, что рубрика прячет
  (артефакт-сдача t2 = ноль файлов, боевое срабатывание детектора
  t-132; kill rate 37 тестов < 22 тестов). Большой диет-прогон —
  по каденции (релиз/слово), состав диеты пересобрать по выводам.
  РЕЗЕРВ — генератор статического сайта из markdown. Третье
  подтверждение no-op validator'а вне git — фикс в приоритете
  очереди.
