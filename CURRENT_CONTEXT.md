...
крупнейший кусок, порядок стройки 1–7). ОТКРЫТО У ОПЕРАТОРА:
(а) подтверждение MIT (В4 «да» без текста лицензии); (б) подпись
закрытия Phase 1/1.5 (блок в ROADMAP). Прочие нити: вторая
калибровка ~07-18; статья draft-C и White Paper v0.2.0 — у
оператора; остаток старой очереди — on-touch/evidence-gated.
