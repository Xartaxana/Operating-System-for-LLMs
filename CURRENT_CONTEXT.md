...
БАТЧ ФИКСОВ 5б + ТРЕТИЙ РЕЛИЗНЫЙ СНИМОК (приказ оператора
2026-07-12 «да это в приоритете», перекрыл прежний порядок; A3 —
следующим после батча). Состав батча — см. пункт очереди ниже,
расширенный тем же словом оператора: (5) код-гейт на ярус автора
механизменного коммита СЕЙЧАС (решение оператора: одной утечки
достаточно, механизм передаётся другим — D-0072 тем же коммитом);
(6) правило независимого воспроизведения в critic.md обеих копий
(ответ на вопрос оператора: роль не командовала то, что экзамен
проверяет; подсказка удачного ретрая = недостающий текст роли) —
триггерит перепрогон critic-экзамена ДО коммита (правило 10
critic.md, inline-метод с пометкой). Стадия 5б ЗАКРЫТА 2026-07-12 (ACCEPTED; установка в
...
defect_found (наблюдение калибровки) + из батча 5б: ретро-пара
журнала (М4, их CLAUDE.md + log_append.py), critic правило 12
«независимое воспроизведение» (М6), tier-строка в их
scripts/mechanism_gate.py (D-0072, М5).
...
- БАТЧ ФИКСОВ НАХОДОК 5б (Lead-class, механизмы — полный rule-10
  на каждом; WHEN: после A3 либо словом оператора раньше;
  рекомендация Lead: батч + третий релизный снимок ДО раздачи
  шаблона): (1) F-35 — шаг onboarding «оси принимающего проекта»
  (scout-обзор кандидатов по умолчанию, оператор — судья; его
  вопрос — только про невидимое изнутри репо); (2) F-36 — живая
  секция Lead-очереди в шаблонном CURRENT_CONTEXT + разделение
  «черновик осей — любой координатор, запись в карту — Lead/
  оператор»; (3) регламент ретрая экзаменов (hinted-ретрай ≠ PASS,
  форма записи в Runs log); (4) утечка журнала в свежей установке
  (класс F-9, 3 экземпляра): onboarding явно журналирует
  экзаменационные диспатчи + напоминание о журнале в первом
  делегированном цикле — форму решает rule-10 разбор.
