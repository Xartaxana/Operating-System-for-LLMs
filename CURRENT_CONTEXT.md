...
- БАТЧ МЕЛОЧЕЙ 07-28 (D-0081, копится к границе этапа): (а) тюнинг
  negative_lint — skip async_launched-метаданных Agent-вызовов
  (кандидат обкатки, рецидив выше) + тесты границы skip-условия;
  кит-твина линта нет (кандидатура в кит в обкатке) — ось 7 н-п.
...
- ОЧЕРЕДЬ КАЛИБРОВКИ №5 (~07-30, консолидирована диетой 07-24):
  ...
  атрибутивный прогон на окне №5; в окне уже два чек-23 кейса
  (t-324/t-325) + инцидент D-0060 govard (протухание проверки
  ...
  parity 40/7/0/3 все записанные).
