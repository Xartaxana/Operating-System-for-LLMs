...
- **КАЛИБРОВКА №6 — 2026-08-05, ПРОВОДИТ OPUS-СЕССИЯ (слово
  оператора 2026-08-04)** — первый боевой Lead-прогон привязки
  D-0099: сами чеки — Lead-работа, Opus = полный Lead по новой
  матрице. Состав: стандартный протокол
  (PROCESS/WEEKLY_CALIBRATION_PROTOCOL.md) + особые входы этого окна:
  чек 5 впервые по якорю delegation.config.yaml (не имени Fable);
  чек-23 копилка — три кейса 2026-08-04 (позиция ветки в спеке
  t-350, неназванный край «конфиг появится позже», немашиночитаемая
  owns-строка диспатча); сверка D-0099-хвостов (ниже). Замер
  экономии перепривязки — НЕ в этот прогон (нужно закрытое окно
  14.08–14.09 token_usage.xlsx). Спорные вердикты — в очередь с
  пометкой, добор Fable-резервом только по слову оператора.
...
- **D-0099 хвосты (заведено 2026-08-04, перепривязка Lead→Opus 5):**
  ...
  словом 2026-08-04); (е) батч мелочей builder'у (остатки
...
- **Находка handoff 2026-08-04: неизвестный писатель logs/token_usage.xlsx** —
  ...
  (данные верны, файл дериватив). Расследовать вместе с тестами
  скрипта (пункт ниже): найти писателя, закрыть скрипт от записи в
  реальный xlsx из тестового контекста (guard по GATEWAY_DB_PATH
  или явный --output).

- **Тесты token_usage_stats (ось 5, заведено 2026-08-04)** —
  tools/token_usage_stats.py (месячная статистика токенов → 
  logs/token_usage.xlsx) заведён без тестовой обвязки; добавить по
  паттерну tools/test_usage_report.py (tmp DB, изоляция): оконное
  бакетирование (граница 14-го, UTC→local), идемпотентность append
  (период не дублируется), EARLIEST_WINDOW-отсечка. Батч мелочей
  builder'у на ближайшей границе этапа.
