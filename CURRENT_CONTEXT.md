...
- **РЕВЬЮ CODEX 07-24 РАЗОБРАНО 07-28, части (а)/(б) ИСПОЛНЕНЫ**
  ...
  (6) НОВОЕ 07-29 (Dog-синк п.6): контурная разметка канонических
  команд kit-CLAUDE.md — pytest-канон с gateway/ контур-пометить/
  параметризовать, подписочные потомки наследуют падающую команду;
  (7) НОВОЕ 07-29 (t-339 F7): casing-канал SKILL.md в
  toolkit/tools/wiring_check.py (у кит-твина канала нет);
  (8) НОВОЕ 07-29: заметка «скоупинг условий двухусловных детекторов»
  в руководство детекторов кита (двойная независимая находка — Dog
  D-0009 + наш hygiene_gate FP; снятое предложение 7 синка);
  (9) РАСШИРЕНА 07-29: кит-hygiene heredoc-скраб — COMMIT_HEREDOC_RE
  с F1-фиксами t-339 (скраб только тела, скоуп на git-commit
  statement), дельта твина выросла портом t-338;
  (10) кандидаты по итогам штабной обкатки (класс owns_gate/
  negative_lint): correlation-пара search/claim_control_gate +
  enforcement_probe + wiring_check-CLI паттерн — решение при
  релизном батче;
  (11) НОВОЕ 07-29 (промоция №5): designer-роль (.claude/agents/
  designer.md + мост D-0062) — порт в кит при релизном батче;
  (12) НОВОЕ 07-29 (t-343): given-path warn-слой кит-dispatch_gate —
  той же порт-строкой, что word-boundary класс (п.4а выше);
  (13) НОВОЕ 07-29 (пост-№5, предложение оператора): R11-твин
  «края поведения в спеке» — кит-CLAUDE.md (норма + чеклист п.2)
  тем же релизным батчем.
...
- OWNS_GATE АКТИВЕН с 07-28 (t-332 att.3 принят, хук в
  ...
  манифестом owns без пути в строке.
