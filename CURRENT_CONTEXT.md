...
ПОЛЬЗОВАТЕЛЬСКИЙ C:\Users\user\.claude\settings.json (действует на
все сессии Claude Code, вкл. десктоп, после их перезапуска; бэкап
settings.json.bak-apiwindow рядом). ЗАКРЫТИЕ ОКНА = удалить блок
env из settings.json (или вернуть бэкап) + можно закрыть окно
прокси. ОБЯЗАТЕЛЬНЫЙ шаг: после ПЕРВОГО реального СТРИМИНГОВОГО
запроса окна сверить non-NULL cache_read_input_tokens (критик №1;
смок был нестриминговый).
