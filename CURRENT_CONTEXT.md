...
- ОНБОРДИНГ-АРХИТЕКТУРА (слово оператора 07-22; прецедент —
  отказ AO3 от D-0087 как «признанное отличие», находка F-51):
  драфт docs/tasks/2026-07-22_onboarding-architecture.md ГОТОВ К
  РЕВЬЮ Архитектора — онбординг = экзамен-инвентаризация функций
  хоста (D-0062) → adoption ledger (adopt / native-equivalent /
  deferred / rejected) → замыкание предпосылок → measure-first
  слои; ДВА РЕЖИМА одной процедурой (слово 07-22): greenfield
  (пустой проект — пустая карта = полный комплект, baseline с
  первого дня, ledger как носитель будущих отличий) / brownfield
  (живая схема — сопоставление); режим определяется картой
  инвентаризации, не декларацией. Развилки к слову: чеканка DECISIONS (сейчас vs после ревью
  WP), статус ledger (норма vs шаблон), полигон-валидация третьим
  деплоем. Кит-кандидаты — порт-очередью (мораторий D-0074);
  вторая проходка Lead по AO3 HANDOFF:205–233 — при чеканке
  (D-0066).
