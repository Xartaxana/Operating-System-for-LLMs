...
- ТУЛКИТ: РЕЛИЗ v0.4.0 ВЫШЕЛ 07-20 (публичный d139f3a + тег;
  ...
  следующем старте прокси. НОВАЯ ОЧЕРЕДЬ (находка вопроса
  оператора 07-20 «одинаковые ли правила»: кит отстаёт от штаба на
  4 правила 07-18, не попавшие в порт-очередь): 1) двухслойный
  критик-вход (−37% доказано); 2) батчинг мелочей (D-0081); 3)
  DAG-носитель задач (D-0080); 4) крупная каденция R12. Плюс вопрос
  их Lead'у AO3: читает ли их open-dispatches прозаические закрытия
  (наш старый дефект-класс; closes:-токена у них нет — сканер не
  сверялся). Мораторий в силе до слова.
