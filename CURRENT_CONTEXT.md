...
ПЛАН СЛЕДУЮЩЕЙ СЕССИИ (слово оператора 07-16 ~15:30, приоритет):
1. Приёмка t-159 (kit v5 + staging_hq; builder-сдача в working tree
   kit'а) — CRITIC-ВХОД ОБЯЗАТЕЛЕН (машинные гейты, правило 3);
   пин kit v5.
2. ПЕРЕНОС В ОСНОВНУЮ ВЕТКУ: (а) штабной порт гейтов из
   staging_hq — ревью каждой адаптации, tools/ + активация
   .claude/settings.json, механизменный коммит tier:fable (гейты
   начнут гейтить все штабные сессии, вкл. Lead); (б) «критик на
   план» строкой в штабное правило 3; (в) role-файл critic.md —
   ВСЁ ждущее из очереди «Уроки ярусам» (семантика полей эмпирикой
   F-38; соседние классы поверхности 9**9**9) — касание critic.md
   ТРЕБУЕТ regression-прогона критик-экзамена (D-0057-аналог,
   чек 14) + кейс-кандидаты б/г в CRITIC_EXAM тем же касанием.
3. ПРОГОН №11: малый экзамен (набор №1) с обновлённой основой
   (B = штаб + гейты + новый critic.md) — очередная точка медианы.
