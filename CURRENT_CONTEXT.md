...
- **Кросс-пункт от AO3 2026-07-28 (их калибровка №4, правило 4б/D-0082
  — носитель цели):** словарная дыра валидации `basis` у AO3 закрыта
  полной парой (tier(agent), tier(by)) — их log_append.py, коммит
  30e79c8: членство basis в {critic, queued-to-lead} пропускало
  queued-to-lead на Sonnet-классе (2 прецедента у них) и basis=critic
  на opus-классе (4 легаси-записи). ТВИН-ВОПРОС к эталонной стороне:
  несёт ли ваш валидатор журнала (tools/journal_validator.py /
  проверка by+basis, если есть) ту же дыру «членство вместо пары»;
  решение перенять/признать отличие — за Lead OS. Кандидат в очередь
  калибровки №5.
