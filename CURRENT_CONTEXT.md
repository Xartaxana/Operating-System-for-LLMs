...
- ЭКЗАМЕН ЭКОНОМИИ — СЕРИЯ №1–№4 ЗАКРЫТА, ВСЕ РЕШЕНИЯ ПРИНЯТЫ 07-15
  ...
  калибровке.
- НАБОР №2 — ПИНОВКА ЗАВЕРШЕНА 07-15 (~17:10), ЖДЁТ СЛОВА ОПЕРАТОРА
  НА ПРОГОН. «Трекер расходов из legacy-прототипа», 4 сессии × 3
  плеча, Sonnet-координатор, табель 6б. Пин-док:
  docs/tasks/2026-07-15_economy-exam-set2.md (S1–S4, батарея A 25
  проб, чек-лист B 12 пунктов; поправка: cross-arm чередование
  отменено Rule #1). Фикстура t-131 принята (513b336; sandbox +
  ground truth sha-стабилен). Раннер t-132 принят через critic-цикл
  (761d4f6; sessions:[...] в одном cwd, per-session учёт, блокер
  collect закрыт). Смета $40–90. Дальше: слово оператора → прогон
  07-16..17 → табель/вердикты → строка Runs log → разбор к
  калибровке ~07-18. В копилку разбора: самоотчёт сессии != факт
  диска (смок t-132) — приёмка клеток только по файлам песочницы.
  РЕЗЕРВ на будущее — генератор статического сайта из markdown.
