...
- КАЛИБРОВКА №4 ИСПОЛНЕНА 07-23 (досрочно, слово «запускай»; полный
  ...
  чекам 30/31/экономике ПРОЙДЕН — релиз v0.5.0 ждёт слова
  оператора. ОЧЕРЕДЬ ИЗ НАХОДОК №4: (1) внеплановый прогон
  SCOUT_GOLDEN_SET назначен (recon-реджекты ×10 окна, чек 14в;
  триггер — ближайшее scout-касание или отдельный батч);
  (2) ретро-closes батч 18 pre-window открытых delegated
  (t-029..t-206 — хвост до closes:-парсера t-197); (3) кандидаты
  решений Lead/оператора: явное исключение детерминированных
  скрипт-запусков из skip-требования (чек 22, ×12 за окно), узкие
  git -C allowlist-правила (чек 25 — решение оператора), контроль
  формы негатива в судейскую инструкцию и scout.md (чек 20, кейсы
  t-268/t-272), чек-лист манифеста диспетчера перед отправкой
  (чек 23 — утечка кодификации D-0073); (4) редизайн канала
  witness-эха после снятия tier_echo (defect t-203) — стоит с 07-19,
  подтверждён как открытый; (5) порт-очередь за релизным гейтом
  дополнена: кит-политика rule 13 MAY→default + интент-ключевая
  строка форм-контроля (D-0094).
...
- ВАЛИДАЦИОННЫЙ СЛОЙ N1..N4 + ЧЕКАНКИ D-0091/D-0092 + АУДИТ
  ...
  первый прогон чека 32; релиз кита v0.5.0 — ЗА гейтом №4, staging
  заморожен; порт-очередь за релизным гейтом: обобщённый
