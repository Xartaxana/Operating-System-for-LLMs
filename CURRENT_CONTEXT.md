...
- ТУЛКИТ: ПОРТ-БАТЧ ИСПОЛНЕН И РЕЛИЗ v0.2.0 ВЫШЕЛ 2026-07-14 (слово
  ...
  docs/tasks/2026-07-14_economy-exam-run2.md. На разбор калибровки
  ~07-18: B ×2 валом к №1 (конфаунд релиз+дизайнер — вход в метрику
  2 пилота дизайнера; контрольное B0 запущено 07-14 ~22:00);
  ПОПРАВКА СКОРОСТИ (вопрос оператора): «дрейф базлайна» отозван —
  артефакт смешения методов замера (№1 транскрипт-файлы, №2
  cc_usage; одним методом A/C без изменений, B ×2.1), троттлинг
  параллельных сессий измерен дырами >60s (B №2: 16 мин на плечо);
  кандидат правки протокола — пин метода метрики 4 + фиксация
  параллельности окна.
