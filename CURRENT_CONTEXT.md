...
- ГЕЙТ-ОТЧЁТ ROUTER 2026-07-20 (слово «делай все 4, не жди
  ...
  ПРЕДЛОЖЕНИЕ РЕВИЗИИ КРИТЕРИЕВ (требует DECISIONS-записи, решение
  Архитектора): R2' = координационные накладные >=25% спенда
  конструкции (измерено: ~85%); R4' = предрегистрированные D-ключи
  экономэкзамена (D-плечо: $/принятую <= C при качестве >= B-0.05 и
  wall-clock <= B, медианы >=3 прогонов). Инструментальные вердикты
