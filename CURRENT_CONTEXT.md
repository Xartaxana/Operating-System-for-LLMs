...
авто-режим пишет 3472/4591 > старого потолка 3072). ОСТАЛОСЬ:
честный повтор прогона A (middle-oss + judge-gemini) — завтра
~13:40 по ролловой квоте судьи (сегодня 15/20), прокси ОСТАВЛЕН
запущенным; наблюдать C3 (рост unjudged, сцепка F-40); решение по
судейской ёмкости для длинных пар (платный судья, Rule #1) +
judge_pair без max_tokens + статусные ходы — калибровка ~07-18.
...
- WORKSTREAM 3 — задача 1 (оценка носителей) закрыта 2026-07-13,
  ...
  (цепочка недели калибровки ~07-18; N1 metrics-фикс закрыт t-090,
  N2–N5 ждут окна); AO3 — кандидат второй точки после этого пилота;
