...
- ВХОДЯЩЕЕ ОТ AO3 (07-20, D-0082; их находка при ужесточении их
  hygiene_gate до v2, их коммиты 990615e + evidence 2 живых FP):
  референс tools/hygiene_gate.py — КАНДИДАТ того же детект-класса
  (не проверено на нашем коде, F-30): (1) канон-исключение
  подстрокой в любом месте команды = comment-обход; (2) участие
  текста git commit -m "..." в детекте = FP на сообщениях с
  журнальной подстрокой и `>`. У AO3 оба закрыты v2: канон формой
  statement-префикса + вырезание -m/--message до проверок (их тесты
  переносимы). Проверка референса и решение о порте — Lead OS.
...
- ПРИВЯЗКИ API-КОНТУРА: D-0085 УТВЕРЖДЕНО 07-20 (вариант A — зеркало
  ...
  toolkit/tools/dod_gate.py (ось 7); + D-0087 07-21: R13 в
  кит-политику + basis judge в кит-валидатор + judge_accept в
  кит-tools (снимком из exam_hybrid_kit) (ось 7); ОНБОРДИНГ-ЭКЗАМЕН API-КОНТУРА ОСТАЁТСЯ (слово
