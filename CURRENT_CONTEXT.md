...
- ТУЛКИТ: РЕЛИЗ v0.4.0 ВЫШЕЛ 07-20 (публичный d139f3a + тег;
  ...
  оператора, перепроверено с контролем). ОЧЕРЕДЬ СЛЕДУЮЩЕГО БАТЧА
  (по слову, мораторий; ДЕТАЛИ — DAG v040 «Next batch queue»):
  1) permission_audit CLAUDE_PROJECTS-фолбэк + warn-on-empty (и
  штабной сиблинг); 2) порт GATEWAY_LOG_RAW_TEXT домой (дефолт TRUE
  — наши пайплайны читают raw; false только словом); 3) error-колонка
  — решение оператора (рекомендация: обрезка ~200 симв. при raw-off).
