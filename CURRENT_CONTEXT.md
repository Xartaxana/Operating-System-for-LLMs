...
- ЭКЗАМЕН ЭКОНОМИИ — СЕРИЯ №1–№4 ЗАКРЫТА, ВСЕ РЕШЕНИЯ ПРИНЯТЫ 07-15
  ...
  правило 11 (их сессией или Lead при касании);
  аналитика ретраев/холодного старта и окна прогонов в notes —
  калибровке.
- НАБОР №2 ЗАКРЫТ 07-16 (вердикты внесены; финал: №5 0.64/0.70/0.70,
  №6 0.72/0.86/0.82, $128.69) — ВСЁ повествование, вердикты,
  декомпозиция расходов B и выводы: docs/tasks/
  2026-07-15_economy-exam-runs5-6.md (+ строка Runs log протокола).
  ЖИВОЙ ОСТАТОК: (а) находки в очередь фикса — journal_validator
  no-op вне git (ложный «валиден»; класс enforcement-врёт-ОК-вне-
  среды) + раннер не помечает пустой stdout при rc=0; (б) калибровке
  — пометка синтетики окон экзаменов (D-0075), тренды, счётчик
  возвратов воркеров (правило 11 само-проверка), данные «политика
  на Sonnet не держит вопрос-дисциплину» в матрицу D-0058.
- НАБОР №3 = ДИЕТ-B: ПРОГОН №7 ЗАКРЫТ 07-16 ночью (разбор —
  docs/tasks/2026-07-16_economy-exam-run7.md, строка Runs log есть).
  Итог: качество Bfull=Bdiet=+3 > C=+2; цена C $7.13 < Bfull
  $10.33 < Bdiet $11.33 (одиночный, шум). ГЛАВНОЕ: диета исполнена
  частично — журнал батчем да, критик-каденцию Sonnet-координатор
  ПРОИГНОРИРОВАЛ (поцикловое ревью) — второе за сутки подтверждение
  «Sonnet ненадёжно программируется текстом» (данные в D-0058-разбор
  калибровки). Кандидат диет-v2: МЕХАНИЗАЦИЯ каденции критика
  (конфиг/код, не текст) — дом дизайна diet-b-design.md; большой
  диет-прогон — по каденции (релиз/слово). РЕЗЕРВ — генератор
  статического сайта из markdown.
