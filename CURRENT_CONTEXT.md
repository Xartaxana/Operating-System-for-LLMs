...
- ОНБОРДИНГ D-0090 ОТЧЕКАНЕН 07-22 (все развилки решены словом:
  ...
  docs/tasks/2026-07-22_onboarding-architecture.md. ЖИВОЕ:
  ВАЛИДАЦИЯ — DAG docs/tasks/2026-07-22_onboarding-validation.md:
  ветка B (D:\Dog) — B1 инвентаризация t-268 (в полёте; ФАКТ
  скаута: Dog — наш же деплой c45f997 с нативными расширениями,
  профиль ветки уточняется при приёмке B1); B3+ по слову до
  касания чужого репо; ветка G (пустой полигон) — место G1 ждёт
  слова (предложение D:\Improving_AI\onboard_greenfield). Кит-батч
  (протокол + ADOPTION_LEDGER.md + манифесты предпосылок) —
  порт-очередью по слову (D-0074). WP: очередь ревью Архитектора
  пополнилась §4.3.
