...
- ПОРТ-БАТЧ ИСПОЛНЕН 2026-07-14 (слово оператора 15:20 открыло синк
  и сдвинуло триггер «после Phase 2»): пп.1–8, 11, 12, 14 закрыты
  батчем t-101..t-105 (спека — дизайнер, прогон 4 пилота; манифест
  расхождений — docs/task_reports/2026-07-14_toolkit-v020-divergence-manifest.md).
  ОСТАЮТСЯ в очереди (свои гейты): п.9 ретро-контур (после >=1
  прожитого прогона ~07-18), п.10 экзамен экономии (после снимка
  v0.2.0, чек 14е), п.13 privacy-режимы (дефолт — решение
  оператора), НОВЫЙ п.15: permission-audit (t-107) — после
  проживания в корне. Снимок v0.2.0 — по слову оператора (состав
  предъявлен). Историческая очередь ниже — для трассировки:
...
- AO3-ПОРТ D-0076 — В РАБОТЕ этой сессией (слово оператора 07-14
  15:20, t-100): worker_ref в log_append.py + подкоманда
  open-dispatches (файловый порядок легален в AO3 по построению;
  закон accepted не тащим — reopen легален) + строки политики/SKILL
  при приёмке. Протухший остаток вычеркнут: by/basis +
  continuation/retry в AO3 УЖЕ портированы 07-10 (разведка t-098).
  Пути живой AO3-сессии (журнал/bugs/state) не трогаем (D-0060);
  запись порт-события в ИХ журнал — их сессии.
