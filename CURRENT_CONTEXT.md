...
- ТУЛКИТ: релиз v0.3.0 ВЫШЕЛ 07-16 вечером (снимок 1a6bfe5, тег
  запушен; батч t-169/t-170 + critic ПРИНЯТЬ + находка A-1 закрыта;
  549 passed и в staging, и в снимке; локальный клон —
  D:\Improving_AI\Supervised-Delegation). Батч закрыл: 11а,
  критик-на-план, DoD-батарею, самопроверку диспатчера, critic.md
  13/14, все 5 гейтов АКТИВНЫМИ в шаблоне, replaces_worker в
  валидаторе+политике, чек 19. ОСТАТОК ОЧЕРЕДИ: п.13 privacy-режимы
  — РЕШЕНИЕ ПОЛУЧЕНО (слово оператора 07-16 ~23:30): дефолт
  шаблона = SAFE TELEMETRY (GATEWAY_LOG_RAW_TEXT=false из коробки:
  токены/цены/категории/fingerprint пишутся, сырые prompt/response —
  нет; включение Shadow Evaluation явно требует raw — честная строка
  конфига + предупреждение при старте; TTL/purge — опция при raw);
  реализация (правка gateway-кита + тесты + доки) — первым пунктом
  СЛЕДУЮЩЕГО батча D-0074; п.15 permission-audit (после проживания
  в корне); хвост п.18 (порядок boot-diet в свежей сессии — сверить
  при следующем касании session_context кита).
  Мораторий D-0074 снова в силе: следующая синхронизация — по слову.
- СЕРИЯ №1–№4 ЗАКРЫТА 07-15 (VERBATIM —
  2026-07-15_exam-week-context-closures.md). ЖИВЫЕ ОЧЕРЕДИ:
  window_load LIKE-исключение + ассерт «id без дефисов» (t-126,
  ось 2);
  AO3 — ПОРТ-БАТЧ ЗАКРЫТ 07-16 вечером (их коммиты 3f4014b +
  bc297bc, запушены; t-171/t-172, critic ПРИНЯТЬ): правила 3/11/11а,
  critic.md +12/13, builder.md +7, --replaces-worker + env-guard в
  log_append (59 passed), staging_gates/ НЕ активирован (D-0069 —
  активация их Lead'ом по их слову). ЖИВОЙ ОСТАТОК AO3: не-блокеры
  критика B-1 (writer строже валидатора в same-agent ветке) и B-2
  (мёртвый mkdir + докстринг log_append:258,291); классовая находка
  «__file__-якорные скрипты» (board_sync/queue_snapshot — scout-pass
  их сессией); стейл-номерная самоссылка прошлого порта в их правиле
  независимого воспроизведения; порт-событие в ИХ журнале пишет их
  сессия (D-0060); их критик-регрессия невозможна по построению
  (набор придёт onboarding'ом кита);
...
- ЭКЗАМЕН-СЕРИЯ 07-16 ЗАКРЫТА и РАЗВЁРНУТА (очередь v5 выполнена
  ...
  docs/tasks/2026-07-16_economy-exam-run7.md). ЖИВОЕ: актуальный kit
  v5.1 @ cbd5cd1; медиана малых экзаменов копится калибровками
  (№11 = 0.88); большой прогон — по каденции; РЕЗЕРВ — генератор
  сайта.
