...
3. ПРОГОН №11: малый экзамен (набор №1) с обновлённой основой
   ...
   Живые смоки гейтов ЗАКРЫТЫ в первой же сессии 07-16: file_path-
   контракт dod_track подтверждён (8 edits с путями), main-записи
   agent_id=null подтверждены, Stop-событие main_gate ПОЛУЧЕНО ЖИВЬЁМ
   (первый блок: Edit .gitignore — dotfile без суффикса, fail-closed
   «не doc-only», прогона после правки не было; разблокирован
   зелёным узким прогоном 131 passed — гейт работает по построению).
   ДВЕ НАХОДКИ о гейте (чек 26в) на калибровку ~07-18, обе — цена
   блока один узкий прогон ~3с, Rule #1 может сказать «не чинить»:
   (1) dotfiles-конфиги без суффикса (.gitignore) падают в
   fail-closed — кандидат расширения doc-only; (2) doc-only
   освобождение «целиком или никак» (по построению): одна не-doc
   правка в треке — и каждый последующий doc-edit требует прогона
   ПОСЛЕ себя (второй блок 07-16: .md-правка после зелёного);
   кандидат — инвариант «прогон после последней НЕ-doc-only правки»
   (доки-хвост не требует ре-прогона); менять только механизменным
   касанием гейтов с тестами.
