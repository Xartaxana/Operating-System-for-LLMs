...
- ОЧЕРЕДЬ КАЛИБРОВКИ №5 (~07-30, консолидирована диетой 07-24):
  ...
  окне №5; окно несёт ПЯТЬ чек-23 кейсов (пятый: t-338 07-29 —
  спека утверждала существование tools/wiring_check.py по кит-строке
  CURRENT_CONTEXT без сверки с носителем, вскрыто возвратом билдера,
  чеклист-пункт 5 «свежесть» D-0096; t-324/t-325/t-332×2 —
  ...
  твин-ответа AO3 ИСПОЛНЕНА 07-29 порт-батчем d4ff7d4 (t-341/t-342:
  casing-канал + --check + enforcement_probe в их pre-commit,
  env-строка builder п.10, R4-твин owns-на-узлах в 4а, HANDOFF-блок;
  E4-scout решён ПРИЗНАННЫМ ОТЛИЧИЕМ — их п.6 сам предписывает
  device-проверки шеллом, Bash оставлен, слово их Lead вольно
  пересмотреть — примечание оси 3 карты).
