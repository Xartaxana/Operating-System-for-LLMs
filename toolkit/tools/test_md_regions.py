"""Battery for tools/md_regions.py. Module not yet imported by any
consumer (a separate later dispatch lands the first consumer) -- this
file is the module's only test.

Run: python -m pytest tools/test_md_regions.py -q

Structure (DoD keys 1-5, mapped to test groups below by a comment
marker "KEY N"):
  KEY 1 -- surface pin (A1): dir() pin + reserved-kinds pin.
  KEY 2 -- A2 invariants: coverage, offsets/CRLF-CR-BOM, nesting stack,
           pure function -- one test group per invariant.
  KEY 3 -- edges, by letter: edge_a..edge_f.
  KEY 4 -- limits, two tests per limit (at the boundary / over it).
  KEY 5 -- module-part battery + perf measurement (witness for the
           "full scan of 1MB <= 150ms" budget).
Plus: A3 CommonMark-lite recognition/deviation pins, A5 fail-open
cases, kind_at()/line_kinds() dedicated tests.
"""
...
# ===================================================================
# KEY 1 -- surface pin (A1)
# ===================================================================


def test_public_api_surface_is_facts_only():
    """Verbatim dir() list (non-underscore names). A new public name
    (should_ignore/is_quoted_claim/SILENCE_KINDS/any policy name)
    changes this list and fails the test -- the detector for "a
    guard's policy leaked into the scanner" (A1)."""
    expected = [
        "KIND_BLOCKQUOTE",
        "KIND_EMBEDDED_DOC",
        "KIND_FENCED",
...
def test_reserved_kinds_never_returned_by_scan_v1():
    """KIND_NOTES/KIND_EMBEDDED_DOC exist as constants, but scan() never
    returns them -- for any text in the corpus."""
...
# ===================================================================
# KEY 2 -- A2 invariants (one test per invariant)
# ===================================================================

# --- invariant 1: full, non-overlapping coverage ------------------
...
# --- invariant 2: offsets in the source text; CRLF/CR/BOM/no-\n -----
...
# --- invariant 3: outer-to-inner nesting stack ------------------
...
# ===================================================================
# KEY 3 -- edges, by letter (a/b/c/d/e/f)
# ===================================================================


def test_edge_a_quote_marker_inside_fence_is_not_a_quote():
    """(a) '>' inside a fence -- no quote."""
...
def test_edge_b_fence_in_quote_closes_at_quote_end_unterminated():
    """(b) a fence inside a quote -- ("blockquote","fenced"); not closed
    before the quote ends -> closes at the quote's end, unterminated=True."""
    text = "> ```\n> line still quoted\n> another quoted line\nend of quote"
    result = mdr.scan(text)
...
def test_edge_c_unterminated_fence_no_verdict():
    """(c) an unterminated fence -- the module reports unterminated, it
    renders no verdict (no separate 'verdict'/'violation' field -- the
    field set is exactly A2's)."""
...
def test_edge_d_inline_code_inside_prose_yields_prose_inline_prose_spans():
    """(d) inline code inside prose -- prose|inline_code|prose spans,
    coverage holds."""
...
def test_edge_e_line_kinds_carries_only_the_block_stack_not_inline():
    """(e) line_kinds carries only the block stack, inline is only
    visible through scan()."""
    text = "plain text\ntext with `code` here\n"
...
def test_edge_f_degraded_means_all_prose_not_a_policy_verdict():
    """(f) degraded = "all prose" -- not a policy, a refusal to mark up:
    even real quote/fence markers inside collapse into a single prose
    region for the whole text, with no attempt to be "smart"."""
    inner = "> quote\n```\ncode\n```\n"
...
# ===================================================================
# A3 -- CommonMark-lite recognition, explicit deviations (one test
# per deviation named in the module's docstring)
# ===================================================================
...
def test_a3_lazy_quote_continuation_not_implemented_pin():
    """Lazy continuation not implemented: a line without '>' ends the
    quote right away -- full-CommonMark lazy continuation is a
    deliberate gap, pinned here."""
...
# ===================================================================
# KEY 4 -- limits (A4), two tests per limit: at the boundary / over
# ===================================================================
...
def test_a5_line_kinds_mirrors_fail_open_for_non_str():
    """Extension of A5 (see the module's docstring): line_kinds() also
    does not raise on non-str input -- it returns []."""
...
# ===================================================================
# KEY 5 -- module-part battery
# ===================================================================


def test_battery_1mb_opening_fences_time_not_quadratic():
    """1MB of opening fences (time, not quadratic)."""
...
def test_battery_10000_backticks_in_one_line():
    """10,000 backticks in one line (span limit) -- must not hang/crash,
...
def test_battery_surrogates_cyrillic_emoji_offsets():
    samples = [
        "hello `code` world in Cyrillic: привет",
        "a\U0001F600b `\U0001F600` end",
        "emoji fence:\n```\n\U0001F600\U0001F601\n```\n",
    ]
    for text in samples:
...
def test_battery_perf_full_scan_1mb_within_150ms_budget():
    """DoD: full scan of 1MB <= 150ms (budget, measurement printed to the
    witness). Ordinary prose text -- the representative "nothing special"
    1MB document (see also test_battery_1mb_opening_fences_time_not_quadratic
