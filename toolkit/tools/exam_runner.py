"""Economy exam runner: drives an economy-exam polygon
...
- Reuses tools/usage_report.py's import_transcripts()/db_path()/
  transcript_glob()/SCHEMA by import, not copy (see docs/SIBLING_MAP.md
...
- No verdicts and no PROCESS/DEPLOYMENT_ECONOMY_EXAM.md Runs-log write
  ...
  acceptance/verdicts stay with whoever runs the exam.
- prepare()'s git operations (clone+checkout) work identically against
  ...
  repos instead of mocking git out: the fake local repos ARE the mock,
  no separate bypass flag is needed.
- project_slug() mirrors Claude Code's ~/.claude/projects directory-
  naming scheme (see its own docstring; verify the transform against
  your own machine's real ~/.claude/projects listing before relying
  on it in a live run).
- Multi-session tasks: a task may carry 'sessions': [text, ...]
  instead of 'text' -- N SEPARATE headless `claude -p` sessions run
  SEQUENTIALLY in the one (task, arm) sandbox cwd, session N+1
  starting only after session N's rc is known. A non-zero rc stops
  the chain by default (no "continue on error" flag -- YAGNI). Each
  session is a fresh process, not --continue/--resume (a returning
  user opens a new chat; state lives in the sandbox's files, not
  session memory). The classic single-'text' task's run_log/
  stdout-file shape is unchanged (backward compatibility). Note: this
  classic shape is keyed off the RUNTIME session count
  (len(launch['sessions']) == 1), not off which manifest key the task
  used -- a task written as 'sessions': [only_one_text] is therefore
  recorded in run_log.json exactly like a classic 'text' task (flat
  entry, no sessions_total/stopped_early, stdout file without the
  '-sN' suffix), not as a length-1 multi-session record.
"""
...
# ---------------------------------------------------------------------------
# console output safe for a cp1251 console (command hygiene)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# doc <-> runner format sync (PROCESS/DEPLOYMENT_ECONOMY_EXAM.md)
# ---------------------------------------------------------------------------

# The exact '## <heading>' text of the doc's headless-escalation-proxy
# section (see build_launch_plan()'s docstring; a manifest arm's
# 'suffix' for arms with a questioning infrastructure is typically
# built from this section's body). Kept as one constant so a doc
# rename that forgets to update it fails a test immediately instead of
# drifting silently -- see extract_doc_section() below.
HEADLESS_PROXY_SECTION_HEADING = "## Headless escalation proxy"


def extract_doc_section(doc_text, heading):
    """Returns the body text of the markdown section starting at the
    exact '## <heading>' line (matched against the stripped line, so
    trailing whitespace in the doc doesn't matter), up to the next
    '## ' heading or end of document.

    Raises ValueError NAMING the heading when: (a) the heading is not
    found at all, or (b) the heading is found but its body is empty
    after stripping -- both are "broken doc format" cases a caller
    building a manifest from the doc needs to fail loudly on, not
    silently paste an empty string into a manifest field."""
    ...
    for i, line in enumerate(lines):
        if line.strip() == heading.strip():
            ...
            break
    if start is None:
        raise ValueError(f"section heading not found in doc: {heading!r}")
    ...
    if not body:
        raise ValueError(f"section {heading!r} found but its body is empty")
    return body
...
def validate_manifest(manifest):
    ...
    arm/task id containing '-'). The hyphen check closes a latent
    invariant: sandbox_metrics() folds sub-slug projects (a sandboxed
    subagent's own distinct ~/.claude/projects slug) into the parent
    sandbox's metrics via 'project = ? OR project LIKE ?' with pattern
    <slug>-%; that LIKE only correctly picks out genuine sub-slugs of
    THIS arm/task and not an unrelated project's slug when the raw
    arm/task id itself is hyphen-free -- a hyphenated id could make a
    legitimately different project's slug LIKE-match as if it were a
    sub-slug. Checked on the raw ids at validation time (not on the
    composed project slug, which always contains hyphens from path
    separators regardless). Mutates manifest in place only to fill the
    'parallel' default (1) when absent. Returns the manifest for
    chaining."""
    ...
    for arm in manifest["arms"]:
        ...
        if "-" in arm["name"]:
            raise ValueError(
                ...
                f"<slug>-%) only distinguishes a real sub-project from an "
...
def project_slug(path):
    ...
    Example: 'C:\\example\\project\\A\\t1' ->
    'C--example-project-A-t1'. Verify this transform against your own
    machine's actual ~/.claude/projects listing before relying on it
    in a live run -- the naming scheme is Claude Code's own and may
...
def build_launch_plan(manifest):
    ...
    compatibility with callers/tests reading the classic single-text
    field).

    A task's 'sessions' list (a multi-session task) is an alternative
    to 'text', not additive (enforced in validate_manifest). All N
    ...
    PROCESS/DEPLOYMENT_ECONOMY_EXAM.md's 'Headless escalation proxy'
    section), extended to every session instead of only the first."""
    ...
    for task in manifest["tasks"]:
        ...
        for arm_name in manifest["order"][task_id]:
            ...
            # masked in tests, which mock the spawn). shutil.which
            # honours PATHEXT; fall back to the bare name so dry-run
            # plans still build on machines without claude.
            ...
            # the first newline -- an early bug delivered ONLY the
            # arm's prefix (task text after \n\n silently lost; proven
            # from the affected sessions' own transcripts). stdin
...
def _ensure_src_repo(dest, source, ref, actions):
    ...
    how tests exercise this exact function offline: fake local git
    repos ARE the mock)."""
    dest = Path(dest)
...
def _template_source_map(manifest):
    """Resolves every template source the manifest references (an
    optional per-arm template_git/template_ref override, layered on
    top of the manifest-wide default) into two dicts:
    ...
    - `sources`: dest Path (under polygon_root/_src) -> (git, ref) for
      ...
      clone), matching "for each unique source, own clone". The
      default is always included, even if no arm ends up using it
      (matches the behavior of unconditionally ensuring _src/template
      when no arm carries an override).
...
def prepare(manifest, dry_run=False):
    ...
    An arm with layout=='template' may carry its own template_git/
    template_ref, overriding manifest.src's default for THAT arm only
    (comparing two template variants in one exam run).
    ...
    dir and VERIFYs each one's HEAD, same as the single default
    _src/template already does when no arm overrides it. An arm
    without an override reads the default _src/template exactly as
    before (byte-identical behavior).

    needs=todo requires a non-empty src['fixture_dir']: a missing or
    empty fixture directory is a NAMED refusal (RuntimeError naming
    the path and the task id) rather than a silent no-op -- a fresh
    deployment pointing at a fixture directory it never populated
    should fail loudly at prepare() time, not produce sandboxes
    missing their task fixture with no signal anywhere downstream."""
    polygon_root = Path(manifest["polygon_root"])
    src = manifest["src"]
    actions = []
        ...
        for arm in manifest["arms"]:
            ...
            if "click" in needs:
                ...
                if not (click_dest / ".git").exists():
                    ...
                    # PermissionError (empirically found running the
                    # idempotency test twice on Windows -- shutil.Error
                    # / [Errno 13] on .git/objects/**). Skip-if-present
                    # avoids the touch entirely, matching the
                    # idempotent pattern of _ensure_src_repo(). The
                    # skip must NOT be silent about drift, though: a
...
def _run_one_session(cmd, cwd, text, polygon_root, arm, task_id, filename_suffix):
    ...
    path and the multi-session path are built from. Returns the same
    {start_ts, end_ts, rc, stdout_tail, stdout_file} shape either way;
    the caller attaches order_index/task_id/arm/cwd (classic) or
    ...
    nothing; the key is omitted entirely when stdout is non-empty, so
    the classic run_log shape is unaffected."""
...
def _execute_launch(launch, polygon_root):
    ...
    launches keep the EXACT classic run_log shape (order_index,
    task_id, arm, cwd, start_ts, end_ts, rc, stdout_tail, stdout_file)
    and stdout filename (<arm>-<task>.txt) -- backward compatibility.

    Multi-session launches run every launch['sessions'] prompt
    SEQUENTIALLY in the SAME cwd -- session N+1 starts only after
    session N's rc is known -- each as a SEPARATE headless `claude -p`
    process (fresh context per session: a returning user opens a new
    chat, not --continue/--resume). A non-zero rc stops the chain by
    default (remaining sessions do not start); the aggregate carries
    per-session records under 'sessions', the last executed session's
    rc as the top-level 'rc', and an explicit 'stopped_early' flag
    (True whenever fewer sessions ran than were planned) so the
    run_log makes the stop visible without the reader having to infer
    it from a short 'sessions' list."""
...
def run(manifest, dry_run=False):
    ...
    -- sequential, clean speed metric). Non-dry-run writes
    ...
    <polygon_root>/stdout/<arm>-<task>.txt); a multi-session launch
    (task['sessions']) instead carries a 'sessions' list of that same
    per-session shape (stdout at
    ...
    carries "empty_stdout": true, regardless of rc; the key is absent
    when stdout is non-empty. Concurrency is bounded across LAUNCHES
    (distinct (task, arm) sandboxes); the sessions WITHIN one
    multi-session launch always run sequentially in the same cwd
    regardless of manifest['parallel']."""
    ...
    if parallel == 1:
        ...
        # empirically scrambled 20/20 under contention, and the
        # rotation order is a pinned protocol requirement ("no
        # favourite last arm").
...
def stall_estimate(turns):
    ...
    overhead), clamped at 0. Rationale: a raw wall-clock gap between
    two turns is not itself 'stall' when the turn's own output
    generation plausibly explains most of the gap -- only the portion
    exceeding the expected generation time counts as stall; this
    avoids conflating a slow-but-working generation with genuine idle
    time. Returns seconds (float)."""
    parsed = sorted(
...
def sandbox_metrics(conn, project):
    ...
    plain equality match would miss entirely. Precedent: a sandboxed
    session's rerun landed partly under such a sub-slug and
    undercounted the sandbox's true cost until this LIKE match was
    added."""
    rows = conn.execute(
...
def window_load(conn, exclude_projects, window_start, window_end):
    ...
    Sub-slug exclusion: a sandboxed session's own subagent can log
    under a distinct '<exclude_project>-<suffix>' project slug (same
    mechanism sandbox_metrics folds into its OWN sandbox's metrics via
...
def run_dossier_tests(sandbox_dir, baseline_files):
    ...
    for each (this genuinely executes the file, not a claude
    invocation), and returns a list of {file, rc, output_tail}. A run
    ...
    Precedent: without this scoping, dossier test runs swept in
    click's own upstream test_*.py files (click/tests/test_*.py) that
    were already part of the prepared baseline, not written by the
    session -- pure noise, some failing against the sandbox's system
    Python package rather than any session defect."""
...
def _run_log_stdout_files(entry):
    ...
    multi-session entry ('sessions'[i]['stdout_file']).
    ...
    entire multi-session run (a real regression caught in review
    before release)."""
...
def _render_dossier_markdown(dossier):
    ...
    if not warned:
        ...
        for r in warned:
            lines.append(
                f"- {r['arm']}/{r['task_id']}: deliverable = external artifact "
                f"(may be unreachable outside the account it was created in)"
            )
    ...
    lines.append("")
    lines.append("No verdicts here -- acceptance and Runs-log entries stay with whoever runs the exam.")
    return "\n".join(lines)


def collect(manifest, dry_run=False):
    """(1) imports fresh transcripts via usage_report.import_transcripts
    (real ~/.claude/projects glob + gateway db, reused by import);
    (2) per sandbox: turns/cost/side/wall/stall from cc_usage
    ...
    references (one file classic, one per session for a multi-session
    task -- ANY session's stdout carrying the marker warns the whole
    task; falls back to the classic '<arm>-<task>.txt' reconstruction
    when run_log.json is absent/has no matching entry, e.g. a stdout
    file assembled without a run); (3) window load table for OTHER
    projects in the exam's [min..max] ts window; (4) writes
    <polygon_root>/dossier.{md,json}. No verdicts, no Runs-log write
    (that stays with whoever accepts the exam's results).
    --dry-run: no-op (no side effects).

    FRESH-POLYGON short-circuit: when BOTH run_log.json and
    baseline_manifest.json are absent/empty under polygon_root --
    i.e. neither prepare() nor run() has ever produced anything here
    -- there is nothing meaningful to collect; this prints a clear
    message naming the polygon_root and returns None WITHOUT touching
    the transcripts db or writing dossier files, instead of silently
    walking every (task, arm) pair to an all-zero dossier that looks
    like tasks ran and did nothing. A run_log.json with at least one
    entry, OR a baseline_manifest.json (even alone -- e.g. a
    hand-assembled sandbox that skipped run()), takes the normal path
    below."""
    if dry_run:
