"""Battery for tools/judge_calib_sanitize.py: sanitized live-probe
input for the subscription-judge role, gateway/judge_calibration.json's
key fields stripped before the acceptor hands the material to the
judge.
...
two are dropped though the named key-field list is only the first
three), kept-field fidelity, pair_index assignment and order
...
contract (default path against the kit's own repo file, explicit path
argument, usage error, missing/invalid/non-array source file), and the
STDOUT ENCODING fix: output must be correct UTF-8 regardless of whether
stdout is a pipe (subprocess capture, already covered by every
_run_cli()-based test above), a redirected file, or a narrow-codepage
console (cp1251 reproduced directly via PYTHONIOENCODING -- U+2192 in
gateway/judge_calibration.json pair 8's target_response used to raise
UnicodeEncodeError before the sys.stdout.buffer fix).

Run: python -m pytest toolkit/tools/test_judge_calib_sanitize.py -q
"""
...
def test_sanitize_also_drops_model_name_fields():
    # the named "kept fields" enumeration lists only prompt/
...
def test_cli_default_path_runs_against_real_repo_file():
    ...
    assert len(out) == 13  # toolkit's own gateway/judge_calibration.json
...
# ---------------------------------------------------------------------------
# STDOUT ENCODING: output must be correct UTF-8 whether stdout is a pipe,
# a redirected file, or a narrow-codepage console -- see module docstring
# "STDOUT ENCODING" in judge_calib_sanitize.py itself.
# ---------------------------------------------------------------------------
...
def test_real_calibration_set_contains_the_reported_non_ascii_character():
    ...
    # the exact character class a narrow-codepage console crashes on. If
    # this ever stops being true the regression test below would pass for
    # the wrong reason (nothing non-ASCII left to trip on) -- pin the
    # premise explicitly.
    with open(REAL_CALIBRATION_PATH, "r", encoding="utf-8") as fh:
        real_pairs = json.load(fh)
    assert any(
...
def test_cli_survives_narrow_codepage_console_environment():
    # regression test for the exact reported failure class: PYTHONIOENCODING
    # set to a narrow codepage (cp1251) used to raise UnicodeEncodeError on
    # U+2192 via a plain sys.stdout.write(); the sys.stdout.buffer byte-write
    # path must be immune to this environment variable entirely (module
    # docstring: "no environment precondition").
    env = dict(os.environ)
...
def test_cli_pipe_file_and_narrow_console_all_agree_byte_for_byte(tmp_path):
    # the three redirection targets named in the edge clause must produce
    # IDENTICAL bytes -- pipe (capture_output), file, and a narrow-codepage
    # console environment (env var only changes the console's notional
    # encoding, not the actual OS-level pipe/file transport in a subprocess
    # test, but it exercises the code path that would differ if
    # _write_stdout_utf8() ever regressed to text-mode writes keyed off
    # sys.stdout.encoding).
