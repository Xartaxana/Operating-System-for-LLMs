"""Shadow-replay harness for Lead-tier dispatch_skipped work.
...
loading a corpus JSONL, pulling each candidate's pre-image and
reference (git-diff ground truth) out of the repo, and formatting/
recording the evidence lines -- never a request to the gateway that
bypasses shadow_eval's own two functions.
...
Usage:
    ...
    python lead_replay.py --only 1,4 --target-model builder
"""
...
# Both streams: candidate prompts/diffs can be non-ASCII and go to both
# stdout and stderr (adversarial-battery error messages too) -- without
# reconfigure, some Windows consoles corrupt them (same class as the
# kit's other console-facing scripts). errors="replace" (not the bare
# default "strict") avoids a second-order UnicodeEncodeError on an even
# narrower console; the CLI prints diagnostic text, not binary data, so
# a replaced character costs nothing but readability at the margin.
...
class LeadReplayError(Exception):
    ...
    traceback (adversarial-battery requirement of the DoD)."""
...
def git_preimage(commit_hash: str, path: str) -> str | None:
    ...
    commit -- the new-file case: callers must treat None as "new file, empty
    pre-image", not as an extraction error.
    ...
    larger repo rather than a repo root itself (verified empirically: this
    toolkit's own nested-dogfooding deployment inside the HQ repo hits
    exactly that divergence -- the same bare path resolved to the OUTER
    repo's unrelated same-named file instead of this deployment's own)."""
...
def build_target_prompt(candidate: dict, preimages: dict) -> list:
    ...
    affected path's pre-image ("given"), with an explicit instruction to
    ...
    parts.append(
        "Below is the current content of the affected files (before the "
        "change). For new files the pre-image is empty (the file does not "
        "exist yet)."
    )
    for path in candidate["paths"]:
        content = preimages.get(path)
        if content is None:
            parts.append(f"=== {path} (new file, empty) ===\n")
        else:
            parts.append(f"=== {path} ===\n{content}")
    parts.append(
        "Return the changed/new files IN FULL (each file's complete content "
        "after the edit, not a diff), with the path stated explicitly before "
        "each file's content."
    )
...
def _truncate_for_display(text, limit: int = _DISPLAY_TRUNCATE_CHARS) -> str:
    ...
    adversarial battery, "giant diff" case). The underlying data handed to
    build_target_prompt/replay/judge_pair is NEVER truncated -- only what
    gets printed here."""
    if text is None:
...
def format_summary_line(date: str, target_model: str, judge_model: str, results: list) -> str:
    ...
    # verdict=="error" also covers an unparseable judge verdict when
    # error=None -- otherwise SUMMARY would show errors=0 while error-
    # marked candidate lines exist, misleading a later reader.
...
def append_replay_evidence(text: str, date: str, target_model: str, entries: list) -> str:
    ...
    under a fresh '### SHADOW-REPLAY (...)' subheading, at the tail of
    docs/SHADOW_EVALUATION_LOG.md. Reuses shadow_eval's own H1 ensure-logic
    (append_evidence_log with an empty entries list touches only the
    heading) so a fresh/empty log file still ends up with the correct
    '# Shadow Evaluation Log' H1 shadow_eval/metrics both key off of,
    without this module reimplementing that heading-creation branch."""
    text = shadow_eval.append_evidence_log(text, [])
    subheader = (
        f"### SHADOW-REPLAY ({date}, target={target_model}, "
        "ground truth = git diff of Lead's own commit)"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Shadow-replay Lead-tier dispatch_skipped work against its own git diff"
    )
