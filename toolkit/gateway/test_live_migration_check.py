"""DoD witness: live migration check on a copy of gateway/requests.db.
...
Run: python -m pytest toolkit/gateway/test_live_migration_check.py -v -s
...
No API key or network access is used anywhere in this file; it skips
outright when toolkit/gateway/requests.db is absent (the kit ships no
committed requests.db -- it is created on first proxy request), which
is the expected state for a fresh subscription-contour checkout.
"""
...
def test_live_migration_adds_category_column_to_real_db_copy(tmp_path, monkeypatch):
    ...
    if not src.exists():
        import pytest
        pytest.skip("toolkit/gateway/requests.db not found (skip in CI without a real DB)")
