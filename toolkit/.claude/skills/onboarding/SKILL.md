...
3. **Adoption ledger.** Copy `ADOPTION_LEDGER.template.md` into your
   ...
   | Any hook-backed gate (mechanism_gate, dispatch_gate, owns_gate, dod_gate, main_gate, hygiene_gate, journal_echo, claim_control_gate, search_control_gate, negative_lint — the last three mechanize command-hygiene point 6's claim/search/negative-form control) | `git config core.hooksPath` pointing at `.githooks`, wired into `.claude/settings.json` | `deferred(trigger: hooksPath + settings.json wired)` or `native-equivalent` (the host's own CI/lint gate already closes the same function) |
   | Hook liveness probe (hook_liveness_probe) | the hook-backed gates it feeds a triggering payload to (row above) already installed — it proves THEIR gate-code liveness by invoking each script directly, not the harness's own wiring (that stays the wiring-integrity-check row's job) | `deferred(trigger: hook-backed gates installed)` |
   | Owns verification, layer 2 (owns_verify) | its layer-1 companion (owns_gate, in the hook-backed-gate row above) already installed — a post-batch CLI re-check of a manifest defense that isn't enforced anywhere without the PreToolUse layer first | `deferred(trigger: owns_gate installed)` or `native-equivalent` (the host's own parallel-dispatch isolation already covers this) |
   ...
   | Calibration / usage accounting (usage_report, savings_report, calibration_counts, token_usage_stats — the last one reuses usage_report's own price table and cost formula, not a second ledger) | a carrier for prices/usage the tooling can read (cc_usage or an equivalent cost record) | `deferred(trigger: usage carrier configured)` |
   | Judge-calibration sanitize feed (judge_calib_sanitize) | the SAME labeled calibration set as the leaf-routing-judge-acceptance row above (gateway/judge_calibration.json) — this mechanism only strips its key fields for a live probe, it creates no second copy of the data | same as the leaf-routing-judge-acceptance row: `native-equivalent` or `deferred(trigger: calibration run)` |
   | Lead-replay shadow evaluation (lead_replay) | contour includes `api-keys` (reuses gateway/shadow_eval.py's proxy-facing primitives) AND a populated corpus — the kit ships only a SYNTHETIC `gateway/lead_replay_corpus.template.jsonl`, never real traffic; the host fills its own corpus from its own history before a real run | `deferred(trigger: contour changed)` under `subscription`-only; `deferred(trigger: corpus populated from the template)` once the contour is `api-keys`/`both` |
   | FINDINGS form + machine check (docs/FINDINGS.template.md, test_findings_form) | a findings log actually created at the host's own path (e.g. docs/FINDINGS.md) with entries following the template's required section markers — the machine check has nothing to validate against an empty/absent log | `deferred(trigger: findings log created from the template)` |
