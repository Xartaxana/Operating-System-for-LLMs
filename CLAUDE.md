...
11. DoD в каждом диспатче (D-0054): что значит «готово» и как
   ...
   + проверочный прогон, чей вывод станет witness (D-0052); scout —
   ...
   Диспатч без DoD — и пишущий/параллельный без манифеста — воркер
   возвращает вопросами, не начиная работу.
...
(D-0076/F-44): `delegated`/`escalated` — ПОСЛЕ возврата
диспатч-вызова, тем же ходом; новый `delegated` несёт `worker_ref`
...
повторный delegated по открытой задаче легален critic-входом или
ретраем с `attempt`>=2 после rejected, на закрытую — запрещён
