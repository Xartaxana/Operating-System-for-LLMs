...
2. Реализация по готовой спеке → builder. Спеку пишет Lead;
   ...
   (или явно ссылается на) фактический вывод проверочного прогона
...
6. Эскалация: 2 неудачные попытки или явный сигнал «не хватает
   ...
   model обязательна; notes: задача, причина, номер попытки, класс
   отказа — spec/capability/recon/tooling; D-0045, D-0052).
...
работы; agent = исходный ярус, notes: что сломалось + ссылка на
исходный accepted — поток false-accept для калибровки, D-0052),
